...
_COMPONENT_CREATE_CONNECTOR_REST_CLIENT = {
    ...
    "template": {
        ...
        "folder_name": "<<optional folder name; metadata only — this platform ignores it on create>>",
        "folder_id": "<<optional folder id; the ONE spelling that places>>",
...
_COMPONENT_CREATE_PROFILE_JSON_GENERATED = {
    ...
    "template": {
        ...
        "folder_path": "<<optional folder path; metadata only — this platform ignores it on create>>",
        "folder_id": "<<optional folder id; the ONE spelling that places>>",
        "description": "<<optional description>>",
        "root": {
            "name": "<<root node name>>",
...
_COMPONENT_CREATE_DOCUMENT_CACHE = {
    ...
    "optional": ["folder_id", "folder_path", "description", "enforce_single_lucene"],
...
_COMPONENT_CREATE_PROCESS_PROPERTY = {
    ...
    "optional": ["folder_id", "folder_path", "description"],
...
_COMPONENT_CREATE_API_SERVICE = {
    ...
    "optional": ["folder_id", "folder_path", "description", "base_url_path", "title", "version"],
...
_COMPONENT_CREATE_CONNECTOR_ACTION_REST_OPERATION = {
    ...
    "runtime_binding_note": (
        "Issue #96 (M5.4a), Boomi UI verified: the REST Client connector step "
        "exposes EXACTLY ONE dynamic operation property — 'Path'. query_parameters / "
        "request_headers here are STATIC operation customProperties (Key/Value, "
        "optionally Encrypted) with NO per-request dynamic-binding mechanism. So a "
        "PER-DOCUMENT path is a process-step concern, expressed via a PATH "
    ...
    "update_note": (
        ...
        "Setting request_profile_type/response_profile_type='none' unbinds "
        "the profile: the new 'none' type applies AND the live profile id "
        "is cleared (no stale id is left dangling beside a 'none' type). "
...
_PROCESS_FLOW_PROTOCOLS = {
    "database_to_api_sync": {
        ...
        "notes": [
            ...
            "with catch_notify: [notify ->] exception. It does NOT compose with a "
            "DOCUMENT-CACHE DLQ route (#184 amendment 3): Add to Cache hands on no "
            "documents, so the Exception after the cache write would never run - the "
            "combination returns PROCESS_EXCEPTION_CONFIG_INVALID on field "
            "reliability.catch_exception, at PLAN time. Nor does it compose with "
            ...
            "= the current document). parameter_source 'none' is refused: the "
            "platform rejects an Exception with no parameter block. "
    ...
    "wrapper_subprocess": {
        ...
        "field_notes": {
            ...
            "process_calls[].wait": "Wait for the child to finish before the parent's own run completes (default true). Nothing follows the call — it ends the path (#175) — so this decides whether the parent waits, not what executes next.",
            "process_calls[].abort_on_error": "Abort the parent if the child fails (default false — the parent run finishes normally instead of failing, matching the live wrapper exemplar). Since the call ends the path (#175), this decides the parent's OUTCOME, not which shape runs after it.",
