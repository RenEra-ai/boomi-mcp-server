...
# Issue #138 M12.3: the process-shape byte serializers were extracted into the
# process_emitters package. The legacy adapters preserve the historical private
# ``_emit_*`` signatures/behavior; the shared rendering module owns the single
# copy of every template plus the layout primitives and emission-domain constants
# (imported back here for the builder's own validation/orchestration, and
# re-exported so existing callers/tests keep resolving these names).
from .process_emitters.legacy import (  # noqa: F401
    _dataprocess_option_tag,
    _emit_branch,
    _emit_catcherrors,
    _emit_connectoraction,
    _emit_dataprocess,
    _emit_dataprocess_combine_body,
    _emit_dataprocess_split_body,
    _emit_dataprocess_step,
    _emit_decision,
    _emit_decisionvalue,
    _emit_doccacheload,
    _emit_doccacheremove,
    _emit_doccacheretrieve,
    _emit_documentproperty_assignment,
    _emit_dragpoints,
    _emit_exception,
    _emit_exception_parameters,
    _emit_flowcontrol,
    _emit_map,
    _emit_message,
    _emit_notify,
    _emit_processcall,
    _emit_property_source_value,
    _emit_returndocuments,
    _emit_setproperties,
    _emit_setproperties_shape,
    _emit_setproperties_step,
    _emit_start_listen,
    _emit_start_noaction,
    _emit_stop,
)
from .process_emitters.rendering import (  # noqa: F401
    _CATCH_DRAGPOINT_Y,
    _CATCH_SHAPE_Y,
    _DATAPROCESS_OPERATIONS,
    _DATAPROCESS_PROFILE_TYPES,
    _DATAPROCESS_SCRIPT_LANGUAGE,
    _DECISION_VALUE_TYPES,
...
# Issue #106 M10.2 / #115 M10.2a: process-level Data Process validation surface.
# The operation metadata (``_DATAPROCESS_OPERATIONS``), script engine
# (``_DATAPROCESS_SCRIPT_LANGUAGE``) and profile-kind set
# (``_DATAPROCESS_PROFILE_TYPES``) the emitters use now live in
# ``process_emitters.rendering`` (issue #138) and are imported above; only the
# validation-only key sets stay here. v1 supports Custom Scripting (Groovy,
# processtype 12), Split Documents (1->N, processtype 8) and Combine Documents
# (N->1, processtype 9); every other documented operation stays rejected with
# PROCESS_DATAPROCESS_OPERATION_UNSUPPORTED until it has a byte-accurate live capture.
...
# ---------------------------------------------------------------------------
# Re-homed to the neutral materializer (issue #153 / M12.15)
# ---------------------------------------------------------------------------
#
# The envelope writer, the override renderer, the extension reader and the two
# process-option strings now live in
# ``categories.components.process_component_materializer`` so the CANONICAL chain
# can reach them without importing this module — a canonical process must resolve
# no legacy ``process_kind``, and importing the legacy builder to borrow its
# envelope writer would have made that impossible to state, let alone test.
#
# These are ALIASES, not copies. All three migrated arms below keep calling the
# same implementations, so the legacy path stays byte-identical and remains the
# parity oracle until #160. ``_extract_process_extension_connections`` in
# particular is imported by ``categories.integration_builder``, so the name must
# stay resolvable here.

_extract_process_extension_connections = extension_bindings_from_legacy_config
_emit_process_overrides = render_process_overrides
_assemble_process_component_xml = assemble_component_xml
_DEFAULT_PROCESS_OPTIONS = DEFAULT_PROCESS_OPTIONS
_LISTENER_PROCESS_OPTIONS = LISTENER_PROCESS_OPTIONS
