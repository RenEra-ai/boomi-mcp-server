...
_CLOUD_ATTACHMENT_PROPERTY_FIELDS = {
    'account_disk_usage', 'as2_workload', 'atom_input_size',
    'atom_output_overflow_size', 'atom_working_overflow_size',
    'cloud_account_execution_limit', 'cloud_account_execution_warning_offset',
    'container_id', 'download_runnerlogs', 'enable_account_data_archiving',
    'enable_atom_worker_warmup', 'flow_control_parallel_process_type_override',
    'http_request_rate', 'http_workload', 'listener_max_concurrent_executions',
    'max_connector_track_docs', 'min_numberof_atom_workers', 'numberof_atom_workers',
    'queue_commit_batch_limit', 'queue_incoming_message_rate_limit',
    'queue_max_batch_size', 'queue_max_doc_size', 'queue_msg_throttle_rate',
    'queue_use_file_persistence', 'test_mode_max_doc_bytes', 'test_mode_max_docs',
    'worker_elastic_scaling_threshold', 'worker_max_execution_time',
    'worker_max_general_execution_time', 'worker_max_queued_executions',
    'worker_max_running_executions', 'worker_queued_execution_timeout',
}
...
def _action_update(sdk: Boomi, profile: str, **kwargs) -> Dict[str, Any]:
    ...
    is_cloud = getattr(current_atom, 'is_cloud_attachment', False)
    ...
    # Cloud attachments don't support purge/restart fields
    if not is_cloud:
        update_data['purge_history_days'] = getattr(current_atom, 'purge_history_days', 30)
        update_data['purge_immediate'] = getattr(current_atom, 'purge_immediate', False)
        update_data['force_restart_time'] = getattr(current_atom, 'force_restart_time', 0)
...
def _action_get_account_cloud_attachment_properties(sdk: Boomi, profile: str, **kwargs) -> Dict[str, Any]:
    ...
    resource_id = kwargs.get("resource_id")
    if not resource_id:
        return {"_success": False, "error": "resource_id (atom_id) is required for 'get_account_cloud_attachment_properties' action"}
    ...
    if hasattr(response, 'result') and response.result:
        ...
        if items:
            item = items[0]
            for attr in _CLOUD_ATTACHMENT_PROPERTY_FIELDS:
...
def _action_update_account_cloud_attachment_properties(sdk: Boomi, profile: str, **kwargs) -> Dict[str, Any]:
    ...
    if not resource_id:
        return {"_success": False, "error": "resource_id (atom_id) is required for 'update_account_cloud_attachment_properties' action"}
    if not request_body:
...
def _action_create_account_cloud_attachment_quota(sdk: Boomi, profile: str, **kwargs) -> Dict[str, Any]:
    ...
    resource_id = kwargs.get("resource_id")
    if not resource_id:
        return {"_success": False, "error": "resource_id is required for 'create_account_cloud_attachment_quota' action"}
    request_body = kwargs.get("request_body")
    if not request_body:
        return {"_success": False, "error": "config.request_body is required for 'create_account_cloud_attachment_quota' action"}

    # Inject cloud_id from resource_id
    if isinstance(request_body, dict):
        request_body["cloud_id"] = resource_id
