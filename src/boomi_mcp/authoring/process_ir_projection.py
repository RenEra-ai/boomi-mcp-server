...
_NODE_FACTS: Mapping[str, Mapping[str, Any]] = {
    ...
    "cache_remove": {
        ...
        _ORDERING: (
            "Work that reads the cache after a removal goes in a later leg of the same "
            "Branch. That leg sees the cache emptied of everything stored before the "
            "removal, a caller's documents included, when the removal runs whenever the "
            "leg does: the removal's own leg hands on unchanged the documents its Branch "
            "received (only messages, maps, flow control or property writes before it), "
            "or its path provably carries documents. A removal behind a step of its own "
            "leg that may hand on no documents, such as a connector call, a cache "
            "retrieve or a data process, may be skipped while the later leg runs, so "
            "everything stored before it may still be read there.",
        ),
        _DOCS: ("required", "consumed", "all_documents"),
        _CAPS: ("keyed_cache",),
        _RELATED: ("state_visibility.cache",),
    ...
    "process_call": {
        ...
        _ORDERING: (
            ...
            "one document can reach the call, a No Data child is refused when it "
            "requires something of a document cache it may also add to or empty, "
            "because a later run may find what an earlier run added or emptied: when it "
            "reads the cache before writing it; and when it consumes the profile or uses "
            "a property of the documents it retrieves, including documents it stored "
            "there itself, unless every run ends with nothing it stored left in that "
            "cache (a whole-cache removal after its last write that runs whenever that "
            "leg does, with no call able to write that cache standing after the removal, "
            "no call it does not wait for able to write it wherever that call stands, "
            "its own or one inside a process it calls, and no outside writer declared for "
            "that cache on a retrieve of it that authors external_writer — such a writer may "
            "refill it between runs, so a cache that has one takes no exemption however the "
            "child is authored, while a declaration no retrieve names establishes nothing "
            "and withholds nothing) and the call is authored "
            "wait=true "
            "and abort_on_error=true. Whether a call may write the cache is read off the "
            "contract derived from the called process: a call whose ProcessIR this request "
            "carries may write it when that contract says it writes it, and a call the "
            "request cannot derive may write any cache the calling process can observe — "
            "so supplying a ProcessIR helps only where the derived contract then names "
            "other caches, and for a call nothing waits for that is the only thing that "
            "helps. A child that reads the cache before writing it has no such way out: it "
            ...
            "and that read is refused whatever the caches do. "
            "Give such a child a Data Passthrough entry and call it wait=true, so it runs "
            "once over the whole group, or call it from a path that hands it exactly one "
            "document. A process property it reads and rewrites is checked for "
            "establishment only: a later run finds it set, with whatever value an earlier "
            "run left.",
            ...
            "reads. A document cache is shared with every caller and Add to Cache "
            "appends, so what a child retrieves includes whatever its callers stored in "
            "that cache, beside anything the child stored there itself. Wherever that may "
            "be so, each call checks its own writes to that cache against what the child "
            "uses of it: the profile a consumer of the retrieved documents requires, and "
            "every document property the child reads, or binds a request path to, after "
            "the retrieve, including one the child's own writes also supply. A caller "
            "that stores nothing in that cache has nothing to check. The obligation "
            "passes up the call chain and ends at the first process on it, the child or "
            "any caller between the child and this call, that removed the whole cache "
            "before the use, or before its own call leading to it, in every run that "
            "reaches it, and has refilled the cache only from its own writes since: a "
            "removal whose leg hands on unchanged the documents its Branch received, or "
            "one on a path that provably carries documents. A removal behind a step of its "
            "own leg that may hand on no documents ends nothing. A call with wait=true "
            ...
            "the child writes on every normal completion at a step its own documents "
            "provably reach. A write the child makes behind a step that may hand on no "
            "documents — a connector call that may return no rows, a retrieve of a cache "
            "that may be empty — establishes nothing for the caller, because a "
            "completion that skipped it is still a normal completion. After any other call with "
...
def _placement_entries(
    ...
    # A placement entry names ONLY the body-placement code (#184 QA-184-s1-r15-01).
    # `authoring_contract_entry_ids_for_diagnostic` cites every entry that names a
    # diagnostic's code, and a placement entry only when the refusal lies in its slot. So a
    # slot entry may name only the code the placement registry's own check raises. The
    # cardinality code is raised by many rules no placement entry decides (an empty try
    # body, a root terminal rule, a root cache successor), and naming it here cited the
    # step entries on refusals they do not explain. A withheld cache step's own refusal is
    # stated on its node page (nothing may follow it) and in its refusal message.
...
def authoring_contract_entry_ids_for_diagnostic(
    code: str, path: Optional[str] = None
) -> Tuple[str, ...]:
    """The entries that explain a diagnostic with ``code`` at ``path``, sorted.

    Every entry that names ``code``, with one exception. A ``placement.*`` entry is cited
    only when the diagnostic's ``path`` lies in that entry's own slot: the INNERMOST
    control-body slot the pointer passes through, read by
    ``body_capabilities.placement_slot_of_pointer`` from the model fields. A path that
    lies in no slot cites no placement entry, and neither does an omitted path.

    Why (#184, defect class ``code-keyed-citation-widened``): citations derived from a
    code alone cited every placement entry naming that code. Every
    ``PROCESS_IR_CAPABILITY_NODE_NOT_ALLOWED_IN_BODY`` refusal therefore cited all ten,
    wherever it was raised and whichever rule raised it. A connector/process_call
    mixing refusal and a recovery-prefix refusal carry the same code, and the pointer
    cannot tell them from a slot-union refusal. They cite the entry of the slot the
    refused node sits in, as a slot-union refusal does. The served entries themselves
    are unchanged; only this per-diagnostic selection narrows.
    ...
    if not code:
        return ()
    from ..compiler.process_ir import body_capabilities

    own_slot = body_capabilities.placement_slot_of_pointer(path)
    return tuple(
        sorted(
            ...
            and (
                entry.entry_type != "placement"
                or (
                    own_slot is not None
                    and [(item.context, item.slot) for item in entry.placements] == [own_slot]
                )
            )
        )
    )
