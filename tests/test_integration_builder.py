...
def _http_conn_config(**overrides):
    """A minimal-valid HTTP connector-settings config dict (Issue #24)."""
    cfg = {
...
def _http_conn_comp(key="rest_connection", name="Target API Connection",
                    action="create", depends_on=None, **config_overrides):
    return IntegrationComponentSpec(
        key=key,
        type="connector-settings",
        action=action,
        name=name,
        config=_http_conn_config(**config_overrides),
        depends_on=depends_on or [],
    )


def _http_send_op_config(**overrides):
    """A minimal-valid HTTP send-operation config dict (Issue #24)."""
    cfg = {
        ...
        "credential_ref": "credential://target-api/bearer",
    }
    cfg.update(overrides)
    return cfg


def _http_send_op_comp(key="rest_send_operation", name="Send Target Record",
                      action="create",
    ...
    return IntegrationComponentSpec(
        ...
        action=action,
        name=name,
        config=_http_send_op_config(**config_overrides),
...
# ---------------------------------------------------------------------------
# Issue #24 — HTTP connector-settings + HTTP send-operation preflight
# ---------------------------------------------------------------------------


def _http_send_op_supporting_components():
...
class TestBuildPlanHttpPreflight:
    """Issue #24 — HTTP connector-settings + HTTP send-op preflight."""

    @patch(_PATCH_TARGET)
    def test_valid_http_connection_plus_send_op_plans_clean(self, mock_pag):
        mock_pag.return_value = []
        components = [
            _http_conn_comp(),
            *_http_send_op_supporting_components(),
            _http_send_op_comp(),
        ]
        plan = _build_plan(MagicMock(), _build_config(components))
        assert plan["_success"] is True
        # Connection comes before operation in execution order.
        order = plan["execution_order"]
        assert order.index("rest_connection") < order.index("rest_send_operation")
        steps_by_key = {s["key"]: s for s in plan["steps"]}
        for key in ("rest_connection", "rest_send_operation"):
    ...
    @patch(_PATCH_TARGET)
    def test_missing_connection_ref_key_marks_send_op_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        op = _http_send_op_comp(depends_on=("target_json_profile", "payload_map"))
        op.config.pop("connection_ref_key")
        components = [*_http_send_op_supporting_components(), op]
        plan = _build_plan(MagicMock(), _build_config(components))
        step = next(s for s in plan["steps"] if s["key"] == "rest_send_operation")
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "MISSING_HTTP_DEPENDENCY"
        assert step["validation_error"]["field"] == "connection_ref_key"

    @patch(_PATCH_TARGET)
    def test_connection_ref_key_not_in_depends_on_marks_step_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        # Drop rest_connection from depends_on — the key is in config but not
        # in depends_on, so execution ordering can't guarantee the connection
        # exists first.
        op = _http_send_op_comp(depends_on=("target_json_profile", "payload_map"))
        components = [
            _http_conn_comp(),
    ...
    def test_request_profile_ref_missing_from_depends_on_marks_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        # Reference $ref:missing_profile but only depend on rest_connection +
        # payload_map. The dependency check must reject before apply.
        op = _http_send_op_comp(
            depends_on=("rest_connection", "payload_map"),
            request_profile_id="$ref:missing_profile",
        )
        components = [_http_conn_comp(), *_http_send_op_supporting_components(), op]
        plan = _build_plan(MagicMock(), _build_config(components))
        step = next(s for s in plan["steps"] if s["key"] == "rest_send_operation")
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "MISSING_HTTP_DEPENDENCY"
    ...
    @patch(_PATCH_TARGET)
    def test_empty_ref_token_in_request_profile_id_rejected(self, mock_pag):
        mock_pag.return_value = []
        op = _http_send_op_comp(request_profile_id="$ref:")
        components = [_http_conn_comp(), *_http_send_op_supporting_components(), op]
        plan = _build_plan(MagicMock(), _build_config(components))
        step = next(s for s in plan["steps"] if s["key"] == "rest_send_operation")
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "MISSING_HTTP_REQUEST_PROFILE_REF"

    @patch(_PATCH_TARGET)
    def test_payload_source_ref_key_missing_from_depends_on_marks_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        op = _http_send_op_comp(
            depends_on=("rest_connection", "target_json_profile"),
    ...
    @patch(_PATCH_TARGET)
    def test_raw_xml_http_connector_settings_skips_builder_preflight(self, mock_pag):
        """config.xml is the raw-XML escape hatch — builder is bypassed."""
        mock_pag.return_value = []
        comp = IntegrationComponentSpec(
    ...
    @patch(_PATCH_TARGET)
    def test_raw_xml_http_send_op_skips_builder_preflight(self, mock_pag):
        mock_pag.return_value = []
        comp = IntegrationComponentSpec(
            key="rest_send_raw", type="connector-action", action="create",
            name="Raw HTTP Op",
            config={
                "component_type": "connector-action",
                "connector_type": "http",
    ...
    @patch(_PATCH_TARGET)
    def test_reuse_path_skips_http_builder_preflight(self, mock_pag):
        """Reuse never invokes the builder — incomplete config must not block reuse."""
        mock_pag.return_value = [_meta(
            "existing-http-id", "Target API Connection", "Process Library",
            comp_type="connector-settings",
        )]
        comp = _http_conn_comp()
        comp.config.pop("url")  # would fail builder validation
        plan = _build_plan(MagicMock(), _build_config([comp], conflict_policy="reuse"))
        step = plan["steps"][0]
        assert step["planned_action"] == "reuse"
        assert step["existing_component_id"] == "existing-http-id"
        assert "validation_error" not in step

    @patch(_PATCH_TARGET)
    def test_update_path_skips_http_builder_preflight(self, mock_pag):
        comp = _http_conn_comp(action="update")
        comp.component_id = "explicit-http-id"
        comp.config.pop("url")
        plan = _build_plan(MagicMock(), _build_config([comp]))
        step = plan["steps"][0]
        assert step["planned_action"] == "update"
    ...
    @patch(_PATCH_TARGET)
    def test_unsupported_auth_mode_marks_http_connection_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        comp = _http_conn_comp(auth_type="BASIC")
        plan = _build_plan(MagicMock(), _build_config([comp]))
        step = plan["steps"][0]
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "UNSUPPORTED_HTTP_AUTH_MODE"

    @patch(_PATCH_TARGET)
    def test_unsupported_method_marks_http_send_op_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        op = _http_send_op_comp(method="HEAD")
        components = [_http_conn_comp(), *_http_send_op_supporting_components(), op]
        plan = _build_plan(MagicMock(), _build_config(components))
        step = next(s for s in plan["steps"] if s["key"] == "rest_send_operation")
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "UNSUPPORTED_HTTP_METHOD"

    # ---- Plaintext-secret scan runs on EVERY HTTP step regardless of apply path.

    @patch(_PATCH_TARGET)
    def test_plaintext_secret_on_http_connection_marks_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        comp = _http_conn_comp(password="LEAK_HTTP_CONN_DEADBEEF")
        plan = _build_plan(MagicMock(), _build_config([comp]))
        step = plan["steps"][0]
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "PLAINTEXT_SECRET_REJECTED"
        assert step["validation_error"]["field"] == "password"
        assert "LEAK_HTTP_CONN_DEADBEEF" not in repr(plan)
        # Plan echo is scrubbed.
        echoed = plan["integration_spec"]["components"][0]["config"]
        assert echoed["password"] == "[REDACTED]"

    @patch(_PATCH_TARGET)
    def test_plaintext_secret_on_http_send_op_marks_unresolvable(self, mock_pag):
        mock_pag.return_value = []
        op = _http_send_op_comp(token="LEAK_HTTP_OP_DEADBEEF")
        components = [_http_conn_comp(), *_http_send_op_supporting_components(), op]
        plan = _build_plan(MagicMock(), _build_config(components))
        step = next(s for s in plan["steps"] if s["key"] == "rest_send_operation")
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "PLAINTEXT_SECRET_REJECTED"
        assert step["validation_error"]["field"] == "token"
        assert "LEAK_HTTP_OP_DEADBEEF" not in repr(plan)

    @patch(_PATCH_TARGET)
    def test_reuse_path_still_rejects_plaintext_secret_on_http_connection(self, mock_pag):
        mock_pag.return_value = [_meta(
            "existing-http-id", "Target API Connection", "Process Library",
            comp_type="connector-settings",
        )]
        comp = _http_conn_comp(client_secret="LEAK_REUSE_HTTP_DEADBEEF")
        plan = _build_plan(MagicMock(), _build_config([comp], conflict_policy="reuse"))
        step = plan["steps"][0]
        assert step["planned_action"] == "error_http_validation"
        assert step["validation_error"]["error_code"] == "PLAINTEXT_SECRET_REJECTED"
        assert step["validation_error"]["field"] == "client_secret"
        assert "LEAK_REUSE_HTTP_DEADBEEF" not in repr(plan)


class TestApplyHttpPreflight:
    """Issue #24 — error_http_validation steps must block apply."""

    @patch(_PATCH_TARGET)
    def test_apply_bails_on_http_validation_error(self, mock_pag):
        mock_pag.return_value = []
        op = _http_send_op_comp(depends_on=("target_json_profile", "payload_map"))
        op.config.pop("connection_ref_key")
        components = [*_http_send_op_supporting_components(), op]
        ...
        assert "rest_send_operation" in unresolvable_keys
    ...
    def test_apply_resolves_request_profile_id_ref_at_apply_time(
        ...
        the send op is executed."""
        mock_pag.return_value = []
        mock_resolve.return_value = []
        # Mock return values keyed by comp.key — execution_order across
        # equal-rank nodes (target_json_profile, payload_map, rest_connection)
        # isn't fixed, so we can't rely on side_effect index ordering.
        component_ids = {
            "rest_connection": "conn-001",
            "target_json_profile": "prof-001",
            "payload_map": "map-001",
            "rest_send_operation": "op-001",
        }
        ...
        result = _apply_plan(MagicMock(), "dev", config)
        assert result["_success"] is True
        # Find the send-op execution call by comp.key and assert that the
        # resolved request_profile_id points at the profile, not the $ref token.
        send_call = next(
            call for call in mock_exec.call_args_list
            if call.kwargs["comp"].key == "rest_send_operation"
        )
        resolved_config = send_call.kwargs["config"]
        assert resolved_config["request_profile_id"] == "prof-001"
