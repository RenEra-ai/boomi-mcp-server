...
| `verified_write_replay_safety` — a stock write action classified replay-safe | gated | #155 — the EVIDENCE landed: seven attested REST captures and one account-scoped operation contract record from a live double execution, and the compiler derives its replay verdict from that observation. The gate stays closed on REACHABILITY: a contract is placed against a complete live identity for a REUSED component, and no authored shape yet both reuses a component and admits a contract |
