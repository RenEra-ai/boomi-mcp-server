...
| `verified_write_replay_safety` — a stock write action classified replay-safe | supported | #155 — classified by OBSERVATION: seven attested REST captures and one account-scoped operation contract record from a live double execution. A retried write still needs an authored contract reference the packaged registry can corroborate, and the operation must be NAMED by component id so its live identity can be read; without either it refuses as before. A plan that creates its components reads nothing extra |
