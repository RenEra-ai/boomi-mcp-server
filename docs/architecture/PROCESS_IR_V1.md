...
| `verified_write_replay_safety` — a stock write action classified replay-safe | supported | #155 — classified by OBSERVATION: seven attested REST captures ingested and one account-scoped operation contract record minted from a live double execution. A retried write still needs an authored contract reference the packaged registry can corroborate; without one it refuses as before |
