...
- **Sibling sweep, with its scope stated rather than implied**: the same invariant is NOT enforced
  repo-wide, because repo-wide it is false today and retrofitting would be churn rather than
  correctness — measured: `issue-152` has 65 archived runs its ledger never cites, `issue-153` has
  11, `issue-175` has 1. Those predate the convention. The guard is therefore scoped to the slice
