...
| **DC-16** | a derived aggregate not recomputed after its source rows changed | the finding rows | **2** — R3-4, R4-4 | second instance was two residual disagreements after the first recompute. Structural response: this table is now REGENERATED from the rows by a script at edit time — the counts above were produced that way, not typed |
...
| **DC-18** | an append-only record edited in place | the ledger's own append-only contract | **1** — R4-5 | originals restored, revisions carried by new rows (D-1a/D-2a). Git history is not the in-tree audit record |

**How these counts are derived.** Each row's defect-class cell is read from the finding
table and tallied, with **revisions applied**: where a revision row supersedes an
original (D-1a over D-1, D-2a over D-2), the revision's assignment wins and the original
is retained above unedited. Rows marked *not an instance* are excluded by that rule, not
by judgement.
