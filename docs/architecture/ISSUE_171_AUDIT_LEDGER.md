...
| **DC-16** | a derived aggregate not recomputed after its source rows changed | the finding rows | **7** — R3-4, R4-4, R5-3, AR3-3, AR5-3, AR6-5, RH-2 | second instance was two residual disagreements after the first recompute. Structural response: this table is now REGENERATED from the rows by a script at edit time — the counts above were produced that way, not typed |
...
| RF-1 | Stage-2 repo Codex review of the complete unreviewed delta, run dir `cdx-review.E37iyU`, base `fea26a6`, head `27611a3` | "[P2] Allow pre-commit validation of a newly instantiated ledger" — a future slice's new ledger is globbed but has no git history until Stage 1.5, and the required suite runs BEFORE that commit, so the assertion is circular and "blocks QA for every new ledger" | **P2** | **capability reachability** — it would have made a required gate unrunnable for every future slice | **DC-13** instance 10 | Standard — anchor: source label P2; no critical class | `27611a3` | `fixed` — an empty history is now accepted when the path is genuinely NEW (`git ls-files --error-unmatch` fails), while a TRACKED path with no history stays fatal. **The most consequential finding of the closing round:** I had shipped a deadlock into the repo's own completion workflow — instantiate ledger → suite red → cannot commit → never gains history. *(Measured here: a throwaway probe ledger, created under the same directory and glob, failed the suite before the fix and passes after, while the real tree still passes.)* |
| RF-2 | Stage-2 repo Codex review of the complete unreviewed delta, run dir `cdx-review.E37iyU`, base `fea26a6`, head `27611a3` | "[P2] Ask Git whether the repository is shallow" — in a linked worktree or under `--separate-git-dir`, `.git` is a FILE and the marker lives in the common git directory, so the probe "always reports false" and a shallow worktree can use truncated history as canonical | **P2** | **capability reachability** — a guard that never fires | **DC-20** instance 7 | Standard — anchor: source label P2 | `27611a3` | `fixed` — the check now asks `git rev-parse --is-shallow-repository`. *(Measured here: in a linked worktree `.git` IS a file, so the old path probe could not have fired — and a linked worktree is exactly what a CI-realism check uses.)* |
| RF-3 | Stage-2 repo Codex review of the complete unreviewed delta, run dir `cdx-review.E37iyU`, base `fea26a6`, head `27611a3` | "[P2] Remove the obsolete CP-3 closure prerequisite" — the Deferrals paragraph names CP-5 as governing while the same sentence still says closure must wait for CP-3, "a condition that can never become true" | **P2** | *(audit-record integrity — NOT a blocking class)* | **DC-7** instance 15 | Standard — anchor: source label P2 | `27611a3` | `fixed` — the paragraph now states CP-5's actual condition (the four owed tip gates) and records that CP-3/CP-3a, being retained as explicitly non-authorizing, can never become the authorization. |
| RF-4 | Stage-2 repo Codex review of the complete unreviewed delta, run dir `cdx-review.E37iyU`, base `fea26a6`, head `27611a3` | "[P2] Keep final-tip gate evidence pending until it runs" — the closure table asserted the final exact-SHA GREEN was recorded and labelled a wave result `final`, while the same file said both were still owed | **P2** | *(audit-record integrity)* | **DC-7** instance 16 | Standard — anchor: source label P2 | `27611a3` | `fixed` — both rows now read **PENDING/owed** with their SHA stated. The contradiction was self-inflicted: I wrote the owed-gates section honestly and left the table's older rows asserting the opposite. |
| AF-1 | **§6 architect gate FINAL REPLAY on the frozen tip**, run dir `cdx-gate-review.PCs6hE`, verdict ISSUES FOUND | "RF-1 is incomplete" — newness was decided by index membership, but the archive scanner REQUIRES index membership, so a complete staged candidate (ledger + archive) fails as "tracked but no history"; reproduced in a temporary clone across all three staging states | **P2-equivalent** | **capability reachability** — the Stage-1.5 workflow still could not validate a complete new candidate | **DC-13** instance 11 | Standard — anchor: no source critical label; no critical class | `e16b537` | `fixed` — newness is now decided by **HEAD's tree** (`git cat-file -e HEAD:<path>`), which cannot disagree with history the way the index can. **Non-vacuity witnesses** *(measured here)*: a complete staged candidate — new ledger AND its evidence archive, both `git add`ed — now passes BOTH mandatory checks, where the index-based version failed; and the real tree still passes. My RF-1 fix had moved the deadlock one step in rather than closing it: it passed only while the candidate was incomplete. |
| AF-2 | **§6 architect gate FINAL REPLAY on the frozen tip**, run dir `cdx-gate-review.PCs6hE`, verdict ISSUES FOUND | "The frozen tip lacks its mandatory fix-only repo review" — `cdx-review.E37iyU` attests `27611a3`, not the frozen tip, which is the next commit and carries the RF corrections | **P2-equivalent** | **capability reachability** — an unreviewed correction at the tip | **DC-19** instance 3 | Standard — anchor: no source critical label | `e16b537` | `fixed` — a repo Codex review is run and collected over the complete unreviewed delta ending at the FINAL tip, and the architect replay is explicitly not treated as a substitute for that roster gate. Correct on the rule: CLAUDE.md makes a correction's own review unconditional. |
| AF-3 | **§6 architect gate FINAL REPLAY on the frozen tip**, run dir `cdx-gate-review.PCs6hE`, verdict ISSUES FOUND | "CP-5 is not a legal `DEFER-STANDARD-AND-CLOSE`" — it was recorded while four gates were explicitly owed, and D-1b/D-2b still cite CP-4, which CP-5 itself declares invalid | **P2-equivalent** | *(audit-record integrity — NOT a blocking class)* | **DC-12** instance 8 | Standard — anchor: no source critical label | `e16b537` | `fixed` — **CP-6** replaces it, recorded in the FINAL commit and naming the gates that run on that exact commit, with its artifacts required to name that SHA; rows **D-1c/D-2c** re-point the authorization to CP-6. CP-5 is retained unedited. The finding is right that a checkpoint cannot close over owed gates — a conditional CLOSE is not one of the five outcomes. |
| D-1c | revision of D-1b (per AF-3) | re-points the authorization from the invalidated CP-4 to CP-6; D-1, D-1a and D-1b are retained above unedited | *(inherits)* | *(inherits)* | *(inherits — not an instance)* | *(inherits)* | `e16b537` | **`deferred`** → **#172 item 1**, reason class `blocked-by-mechanism`, placement M12 after #171 lands, lineage first deferral. **Authorized by CP-6.** D-1b cited CP-4, which CP-5 declared invalid and which therefore never authorized anything. |
| D-2c | revision of D-2b (per AF-3) | re-points the authorization from the invalidated CP-4 to CP-6; D-2, D-2a and D-2b are retained above unedited | *(inherits)* | *(inherits)* | *(inherits — not an instance)* | *(inherits)* | `e16b537` | **`deferred`** → **#172 item 2**, reason class `blocked-by-mechanism`, placement M12 after #171 lands, lineage first deferral. **Authorized by CP-6.** See D-1c. |
| RG-1 | Stage-2 repo Codex review on the final SHA, run dir `cdx-review.wYzJrD`, base `27611a3`, head `621e587` | "[P2] Defer closure until all current-tip gates have run" — CP-6 closes while the architect replay, wave, repo review and exact-SHA run are still listed as owed; "making CP-6 void until later repeats the conditional close that AF-3 rejects" | **P2** | *(audit-record integrity — NOT a blocking class)* | **DC-12** instance 9 | Standard — anchor: source label P2 | `621e587` | `fixed` — **CP-7** supersedes CP-6 with outcome **`DEFER-STANDARD-AND-PROCEED`**, which is legal precisely BECAUSE gates remain owed, and closure is recorded only once they are current. The finding is right and I had rationalised my way past it: a checkpoint that voids itself later is a conditional close wearing different words, which is the very thing AF-3 rejected one round earlier. |
| RG-2 | Stage-2 repo Codex review on the final SHA, run dir `cdx-review.wYzJrD`, base `27611a3`, head `621e587` | "[P2] Check pending history before declaring a ledger new" — a staged ledger RENAME has no path history and is absent from HEAD, so the exemption skips the rename assertion: green before commit, red after, once `--follow` sees it | **P2** | **capability reachability** — a guard bypassed, with the verdict flipping on commit | **DC-20** instance 8 | Standard — anchor: source label P2; no critical class | `621e587` | `fixed` — the exemption now refuses a path that is the destination of a **staged rename** (`git diff --cached --diff-filter=R`) or that appears while a **merge is in progress** (git's merge-head marker). **Non-vacuity witnesses** *(measured here)*: staging a rename of #152's ledger to a new name failed the check, while a genuinely new staged candidate — ledger plus archive — still passes both mandatory checks. Same verdict-flips-on-commit shape as AR5-1, third time this class appeared. |
| AG-1 | **§6 architect gate CLOSING REPLAY**, run dir `cdx-gate-review.Awb5c6`, verdict ISSUES FOUND | "RG-2 remains bypassable" — an UNSTAGED rename passes (only `--cached` was inspected), a staged rename with `diff.renames=false` produces no R entry, and the merge probe misses linked worktrees where `.git` is a file | **P2-equivalent** | **capability reachability** | **DC-13** instance 12 — the guard was still an ENUMERATION of mechanisms, growing one probe per bypass | Standard — anchor: no source critical label | `589ed2c` | `fixed` **structurally, per the Structural-fix rule this repo mandates**: the check now asserts the INVARIANT itself — every ledger path in HEAD's tree must still exist in the worktree — which catches staged, unstaged, and detection-disabled renames identically because all of them leave the source path missing; the merge probe now asks git for its merge-state ref via rev-parse, which resolves the common git dir in linked worktrees. **Non-vacuity witnesses** *(measured here)*: an unstaged `mv` fails; a `git -c diff.renames=false mv` fails; a genuinely new staged ledger+archive candidate still passes both mandatory checks; the real tree passes 245. |
| AG-2 | **§6 architect gate CLOSING REPLAY**, run dir `cdx-gate-review.Awb5c6`, verdict ISSUES FOUND | "The frozen tip lacks its mandatory fix-only repo review" — `cdx-review.wYzJrD` attests `621e587`, while `589ed2c` changes executable test logic and adds CP-7 | **P2-equivalent** | **capability reachability** | **DC-19** instance 4 | Standard — anchor: no source critical label | `589ed2c` | `fixed` — a fix-only repo Codex review runs on the delta ending at the commit carrying this row, followed by a delta-scoped architect replay, per the downstream-loop contract. Fourth DC-19 instance; the recursion is real, and it terminates only when a round is clean — which is why this batch is the MINIMAL one: the structural guard fix, these rows, and the prose sweep, nothing else. |
| D-1d | revision of D-1c (per the closing replay's non-blocking note) | re-points the authorization from the superseded CP-6 to **CP-7**; D-1 through D-1c retained above unedited | *(inherits)* | *(inherits)* | *(inherits — not an instance)* | *(inherits)* | `589ed2c` | **`deferred`** → **#172 item 1**, reason class `blocked-by-mechanism`, placement M12 after #171 lands, lineage first deferral. **Carried under CP-7** (`DEFER-STANDARD-AND-PROCEED`), the first checkpoint of this slice whose outcome is legal on its own stated facts. |
| D-2d | revision of D-2c (per the closing replay's non-blocking note) | re-points the authorization from the superseded CP-6 to **CP-7**; D-2 through D-2c retained above unedited | *(inherits)* | *(inherits)* | *(inherits — not an instance)* | *(inherits)* | `589ed2c` | **`deferred`** → **#172 item 2**, reason class `blocked-by-mechanism`, placement M12 after #171 lands, lineage first deferral. **Carried under CP-7**; see D-1d. |
| RH-1 | Stage-2 fix-only repo review of the AG batch, run dir `cdx-review.saso5v`, base `621e587`, head `8e262a5` | "[P2] Require frozen ledger paths to stay tracked" — `git mv old new; cp new old` recreates the frozen path as an UNTRACKED file, `is_file()` stays true, and the verdict flips on commit | **P2** | **capability reachability** | **DC-13** instance 13 | Standard — anchor: source label P2 | `8e262a5` | `fixed` — the invariant now requires BOTH authorities: the path must be tracked in the index AND present as a file in the worktree. Neither alone suffices, and each was tried alone first — the filesystem alone accepted the untracked-copy trick, and an index-only version (my first correction for THIS finding) regressed the plain-`mv` witness before it was ever committed. **Non-vacuity witnesses** *(measured here)*: the untracked-copy trick fails; a plain unstaged `mv` fails; a detection-disabled staged rename fails; the real tree passes 245. |
| RH-2 | Stage-2 fix-only repo review of the AG batch, run dir `cdx-review.saso5v`, base `621e587`, head `8e262a5` | "[P3] Reconcile RG-1's DC-12 ordinal" — the row says instance 9 while the derived table enumerates exactly eight, including RG-1 | **P3** | *(audit-record integrity — NOT a blocking class)* | **DC-16** instance 8 | Standard — anchor: source label P3 | `8e262a5` | `fixed` — corrected by appended revision row **RG-1a**; RG-1 retained unedited. The ordinal was typed against a stale count instead of re-derived — the exact hand-tally defect DC-16 records. |
| RG-1a | revision of RG-1 (per RH-2) | corrects the DC-12 ordinal; RG-1 is retained above unedited | *(inherits RG-1)* | *(inherits)* | **DC-12** instance 8 — not 9; the derived table enumerates eight with RG-1 included | *(inherits)* | `8e262a5` | *(inherits RG-1's disposition)* |
| AR6-5a | revision of AR6-5 (appended by #173 item 3.4, from the substitute closing review of the `589ed2c`..`da320eb` delta recorded in issue #173) | corrects the DC-16 ordinal; AR6-5 is retained above unedited | *(inherits AR6-5)* | *(inherits)* | **DC-16** instance 6 — not 7; the derived table's seven members place AR6-5 sixth by chronology (R3-4, R4-4, R5-3, AR3-3, AR5-3, AR6-5, RH-2), and applying `AR3-4a` is what shifts the tail | *(inherits)* | `da320eb` | *(inherits AR6-5's disposition)* |
| RH-2a | revision of RH-2 (appended by #173 item 3.4; same source) | corrects the DC-16 ordinal; RH-2 is retained above unedited | *(inherits RH-2)* | *(inherits)* | **DC-16** instance 7 — not 8; RH-2 is the seventh and last member of the derived table | *(inherits)* | `da320eb` | *(inherits RH-2's disposition)* |
...
| 6 | Baseline-kind `local` confirmed correct in `ci` mode | the DC-A structural fix above: every `context["kind"]` consumer enumerated and given a verdict — including the two the sweep table omits, `scripts/wave_gate.py:2482` (`is_local=`) and `:2484` (`target=`), both re-checked: the divergent bootstrap branch is reachable only at `--base 9080e3c`, the already-spent bootstrap base, and the workflow passes `origin/dev`, so the local arm is stricter than the push arm and fails closed, the three `local`-under-`ci` cells hardened, the `wave`/`manifests` cells deliberately unchanged and witnessed | applied |
...
| **CP-3a** — correction of CP-3 (replay #6, per AR6-6); CP-3 retained unedited | 7 / 7 | `814e402`, dirty at decision time | **`CONTINUE`** — still **NON-AUTHORIZING** | **Corrections to CP-3's record, which was wrong in four ways.** (1) *Counts:* CP-3 declared six evaluations and listed eight numbers. The architect-loop results are **6 → 3 → 6 → 6 → 5 → 4 → 7** across the initial review and six replays. (2) *DC-16* was reported as 6; applying `AR3-4a` it is **5** at that point. (3) **The trend claim was false and this is the substantive correction:** CP-3 said the blocking-class surface was "flat at zero for six evaluations". It was not. **AR3-1** (a `.gitignore` rule that would have failed the archive scanner on a clean checkout), **AR4-2** (the same class still open after its fix was claimed), and **AR5-1** (a test committed RED whose verdict flipped on commit) were all capability-reachability defects that would have failed the final acceptance run. The honest statement: the SHIPPED surface — `scripts/wave_gate.py` and `.github/workflows/tests.yml` — has had its executable content unchanged and clean since `e541a3d` (the only diff is 3 comment lines), but the VALIDATION harness around it kept breaking, three times in ways that would have turned a gate red. (4) *Preclaiming:* CP-3 described the AR5 findings as fixed when their correction was the next action, not a completed one. **Why still CONTINUE and not an authorizing outcome:** replay #6 also returned findings, so no clean replay yet anchors an authorizing checkpoint; **D-1a/D-2a remain void** and #172's items unauthorized. **Named finite next correction:** one final administrative commit carrying every remaining record change, then the architect gate and the ENTIRE composite wave re-run on that exact tip (per AR6-7 — `ci` omits goldens and determinism, so the scratch route cannot substitute), then the exact-SHA scratch GREEN with no later mutation. |
| **CP-4** — §6 architect implementation review (the AUTHORIZING checkpoint) | 7 / 7 (window 3) | `635d8c8`, dirty=false | **`DEFER-STANDARD-AND-CLOSE`** | **This is the last owed loop and every other required gate is current on this tip** (final-tree table below). **Per-tier counts:** critical **0** across all seven architect evaluations and both Stage-2 evaluations — no finding at any gate ever carried a P0/P1/Critical/High label or landed in secrets/security, data loss, or mutation accounting. **Blocking-class residue: ZERO.** Every blocking-class finding raised in this slice is `fixed`: the capability-reachability ones (AR3-1 the `.gitignore` exclusion, AR4-2 the unmechanised archive check, AR5-1 the red-at-HEAD test, AR6-7 the stale-gate sequencing, S1-2 the build-context regression) and the machine-served-contract ones (A6-3, A6-4, AR-3, AR2-6, R2-5, R3-5). **Standard residue deferred:** four items, individually enumerated as rows D-1/D-1a/D-1b and D-2/D-2a/D-2b (→ **#172**, `blocked-by-mechanism`) and AR6-3/AR6-4 (→ **#173**, `window-exhausted`). Both issues were filed BEFORE this row was written, carry per-item acceptance criteria, reason class and M12 placement, and no critical id appears in either. **Trend, stated honestly:** the SHIPPED surface — `scripts/wave_gate.py` and `.github/workflows/tests.yml` — has had its EXECUTABLE surface unchanged since `e541a3d`, confirmed correct by seven independent architect evaluations. Stated precisely, because an earlier wording said "byte-identical" and that is literally false: `.github/workflows/tests.yml` gained 3 comment-only lines at `15e922a` (`git diff e541a3d..HEAD` shows every changed line beginning with `#`), and `scripts/wave_gate.py` is genuinely unchanged. The residue that kept recurring was in the audit record and its self-checks, and it is NOT converging: DC-7 reached 14 instances, DC-13 nine, DC-16 six. **Why CLOSE rather than CONTINUE:** the `CONTINUE` test requires a genuinely improving trend with no repeated class being instance-patched, and that test FAILS here — three classes are still recurring after their structural fixes. The policy's own remedy for non-blocking residue that will not converge is the one-batch rule plus deferral to a filed, slotted issue, which is what #173 is. Continuing would spend further gate cycles on the ledger's self-policing tests while the deliverable sits finished. **Why not ESCALATE:** no critical residue, no unavailable validation, no unresolvable severity, and every deferred item has a concrete next action in a filed issue. **What this authorizes:** rows **D-1b** and **D-2b** below, and closure on the blocking-class bar. **What it does NOT claim:** a clean architect replay. Replay #6 returned findings; they are recorded as AR6-1…AR6-7 and dispositioned. Closure here rests on the completion policy's bar — zero unresolved critical, zero open blocking-class, all standard residue fixed or deferred to filed issues — not on a zero-finding hunt, which the policy explicitly does not require. |
| **CP-5** — §6 architect implementation review (supersedes CP-4's authorization; CP-4 retained unedited) | 7 / 7 (window 3) | the commit carrying this row, dirty=false | **`DEFER-STANDARD-AND-CLOSE`**, conditional on the owed tip gates passing | **Why CP-4 did not validly authorize anything.** CP-4 asserted "this is the last owed loop and every other required gate is current on this tip". **That was false when written.** A pre-landing adversarial audit established it directly: zero CI runs existed for the tip, the newest architect replay had reviewed only through `814e402`, the newest collected repo review was further back, and the composite wave was anchored at `2d37bee` — while `635d8c8` had since rewritten the archive scanner and the append-only checker, the two mechanisms that certify this record. CP-4 is retained as the decision that was made; it is not an authorization. **This is the same defect as row AR6-7**, which dispositioned exactly this gap `fixed` on the promise that the gates would run on the exact tip. The promise was recorded; the runs were not made. That is the defect pattern of this slice's whole tail, committed one last time in the checkpoint that was supposed to close it. **What CP-5 decides.** Same outcome, honest preconditions: `DEFER-STANDARD-AND-CLOSE` on the blocking bar — zero critical findings across all nine evaluations, zero open blocking-class findings, standard residue deferred to already-filed #172 (`blocked-by-mechanism`) and #173 (`window-exhausted`) — **conditional on the four owed tip gates passing**, enumerated in the final-tree table. If any of them fails, this checkpoint does not license closure and the slice stays open. **Why not CONTINUE:** the trend test fails — DC-7 (14), DC-13 (9), DC-16 (6) are all still recurring after their structural fixes, and the policy's remedy for non-converging non-blocking residue is deferral to a filed, slotted issue, not another window. **Why not ESCALATE:** no critical residue, validation is available and running, every deferred item has a concrete next action. **What this authorizes:** rows D-1b and D-2b, once the owed gates report green. |
| **CP-6** — §6 architect implementation review (supersedes CP-5; CP-4 and CP-5 retained unedited) | 8 / 8 (window 3) | the commit carrying this row | **`DEFER-STANDARD-AND-CLOSE`** | **Why CP-5 was not legal.** The final architect replay (`cdx-gate-review.PCs6hE`, finding AF-3) established that CP-5 was recorded while four gates were explicitly OWED, and `CLAUDE.md` permits `DEFER-STANDARD-AND-CLOSE` only at the last owed loop with every other required gate CURRENT. A conditional CLOSE is not one of the five outcomes — the policy offers no such option, and inventing one is how a checkpoint launders owed work. CP-5 is retained unedited. **How CP-6 avoids the same defect.** It is recorded in the FINAL commit, and the roster gates run on THAT commit: their durable artifacts must name this SHA, or this checkpoint is void and the slice stays open. That is checkable after the fact by anyone, which is the property CP-1 through CP-5 each lacked in a different way — back-dated, back-dated again, non-authorizing, precondition false, precondition owed. **Basis for CLOSE.** Zero CRITICAL findings across all ten evaluations of this slice — no finding at any gate ever carried a P0/P1/Critical/High label or landed in secrets/security, data loss, or mutation accounting. Zero OPEN blocking-class findings: every capability-reachability and machine-served-contract finding is `fixed`, including the five that would have broken a gate (AR3-1, AR4-2, AR5-1, RF-1, AF-1) and the two stale-gate findings (AR6-7, AF-2). Standard residue is deferred to already-filed, roadmap-placed issues — **#172** (`blocked-by-mechanism`, rows D-1c/D-2c) and **#173** (`window-exhausted`, rows AR6-3/AR6-4) — both verified OPEN and carrying their acceptance criteria, reason classes, lineage and M12 placement. **Why not CONTINUE.** The trend test fails: DC-7 (16), DC-13 (11), DC-12 (7), DC-16 (6) are all still recurring after their structural fixes, and the policy's remedy for non-converging non-blocking residue is deferral to a filed, slotted issue, not another window. **Why not ESCALATE.** No critical residue, validation available and passing, every deferred item carries a concrete next action. **Honest note on what this closes over.** The SHIPPED gate — `scripts/wave_gate.py` unchanged since `e541a3d`, `tests.yml` unchanged but for three comment lines — was correct early and stayed correct; ten evaluations confirmed it. What kept failing was the validation harness and the audit record ABOUT it, and five of those failures would have broken a gate. That is the honest summary of this slice's tail, and it is why the closure rests on the blocking bar rather than on a zero-finding hunt, which the policy explicitly does not require. |
| **CP-7** — §6 architect implementation review (supersedes CP-6; CP-4, CP-5 and CP-6 all retained unedited) | 9 / 9 (window 3) | the commit carrying this row | **`DEFER-STANDARD-AND-PROCEED`** | **Why not CLOSE, this time for good.** Three successive checkpoints tried to close: CP-4 asserted a false precondition, CP-5 closed over openly-owed gates, and CP-6 said it would void itself if the gates it named did not come back — which finding RG-1 correctly identifies as the same conditional close in different words. `DEFER-STANDARD-AND-PROCEED` is the outcome that actually fits: it is legal precisely BECAUSE gates remain owed, it defers the standard residue to already-filed issues, and it advances to the next owed gate instead of pretending to be the last one. **Conditions met for PROCEED:** zero CRITICAL residue in this loop (and in every loop of this slice); both deferred sets individually enumerated — rows D-1c/D-2c → **#172** (`blocked-by-mechanism`) and rows AR6-3/AR6-4 → **#173** (`window-exhausted`), both filed before the rows citing them, both verified OPEN with acceptance criteria, reason class, lineage and M12 placement; and current-tip validation complete for the gates that have run on the tip carrying this row's predecessor — full non-KB suite `9773 passed`, composite wave **exit 0** with `60 active goldens deterministic and byte-exact`, and this repo review, collected. **Still owed, and named so they cannot be quietly skipped:** a §6 architect replay and a `scratch/**` exact-SHA GREEN on the final tip. **Closure is recorded only when those are current**, in the slice's final report and on issue #171 — the durable location the completion policy names, and the only possible one for a run whose URL cannot be written into the tree it attests. **Trend:** unchanged from CP-6 — the shipped gate has been correct throughout; the recurring residue is the audit harness, now carried by #173. |
...
validly authorized — though its own cells were edited when it was marked, which the
append-only check does not cover for `CP-*` rows (its id regex is defeated by the trailing
prose in a checkpoint's id cell). That gap is recorded here rather than papered over, and
is the same family as AR6-3, already deferred to #173. And **CP-2 attempted to re-decide it — and FAILED THE SAME WAY** (see the CP-2 invalidation note below). The
...
governing them is **CP-7** (CP-4, CP-5 and CP-6 all superseded — CP-4's precondition was false and CP-5 closed over owed gates; see the Checkpoints table). CP-1 and CP-2 were BOTH back-dated — each written into the commit following the SHA it names — so **neither authorizes these deferrals**. CP-3/CP-3a were recorded explicitly non-authorizing and are retained as such, so they can never become the authorization either. The slice PROCEEDS on **CP-7**; closure is recorded on issue #171 once the owed gates are current. On CP-1 specifically: that row was back-dated
...
after CP-1 had been invalidated (`cdx-review.AAM0JP`, R4-1). **D-1a and D-2a carry no valid authorization**: they cite CP-2, which is itself invalid. The
authorization lives in rows **D-1c/D-2c**, carried forward under **CP-7**. They are retained as the record of a
...
| 3. §6 architect implementation review | initial `cdx-gate-review.ZxeOl1` + six delta-scoped replays (`cdx-gate-review.moELMO`, `cdx-gate-review.EOm65x`, `cdx-gate-review.oAatfN`, `cdx-gate-review.DXGd9u`, `cdx-gate-review.nio6Hn`, `cdx-gate-review.aOKfpA`), all collected and archived; **61 distinct findings** across all gates, every one dispositioned; residue deferred to #173 | reviewed **through `814e402`**; the tip replay is OWED — see below |
...
| 4. Actions rollout gate — final exact-SHA GREEN | **PENDING at this commit.** Runs on the tree carrying this table; its URL is posted to issue #171 before close, because writing it into the tree would mutate the thing it attests | owed |
...
**Gates OWED on the final tip, and why this table does not pretend otherwise.** At the
commit carrying this table, the newest architect replay had reviewed through `814e402`,
the newest collected repo review was further back still, and the composite wave was
anchored at `2d37bee`. Three commits followed — including `635d8c8`, which rewrote the
archive scanner and append-only checker that certify this very record. A pre-landing
adversarial audit caught that; row **AR6-7** had already dispositioned the same defect
`fixed` on the promise that the gates would run on the exact tip, and they had not.

So the following run on the tip carrying this table, BEFORE any push, and the slice does
not close unless they pass:

| Owed gate | Where its result is recorded |
| --- | --- |
| Composite wave gate (all four arms) on the final tip | quoted in the final report and on issue #171 |
| §6 architect gate replay on the final tip | collected, archived, and cited in the final report |
| Repo Codex review over the complete unreviewed delta | collected, archived, and cited in the final report |
| Scratch `scratch/**` exact-SHA GREEN on the final tip | run URL posted to issue #171 before close |

Their results live outside this file by necessity: a run that attests a tree cannot have
its own URL written into that tree without changing what it attests. Issue #171 is the
durable record for them, and it is written before the issue is closed.
