...
| **DC-16** | a derived aggregate not recomputed after its source rows changed | the finding rows | **5** — R3-4, R4-4, R5-3, AR3-3, AR3-4 | second instance was two residual disagreements after the first recompute. Structural response: this table is now REGENERATED from the rows by a script at edit time — the counts above were produced that way, not typed |
...
| **DC-20** | a file's tracked/ignored/shipped status disagreeing across git, the archive and the build context | what git actually tracks, and what the build context actually uploads | **3** — AR2-4, AR3-1, S1-2 | instance 2 was the one that would have broken a GATE, not a record: `.gitignore` silently excluded the raw logs the checksums listed, and only a clean checkout could see it. Structural response: the archive's file set is now verified against `git ls-files`, not against the working tree |
| **DC-18** | an append-only record edited in place | the ledger's own append-only contract | **4** — R4-5, R5-4, AR2-2, AR3-5 | instance 2 was made WHILE fixing instance 1: D-1/D-2 were restored correctly and the R2 rows touched in the same batch were not. Structural response: an append-only correction now sweeps every row the batch touched, and revisions are appended (D-1a/D-2a, R2-1a…R2-5a) rather than folded in |
...
* **Supersession map** (a revision MERGES onto its original: cells the revision states
  ...
  `D-1a → D-1`, `D-2a → D-2`, `R2-1a → R2-1`, `R2-2a → R2-2`, `R2-3a → R2-3`, `R2-4a → R2-4`, `R2-5a → R2-5`, `INH-TC1-2a → INH-TC1-2`, `R5-1a → R5-1`, `R5-2a → R5-2`. Every mapping is written out: an earlier revision elided the middle of the R2 range with `…`, which reads fine and declares nothing — the append-only check caught it on its first run. So D-1/D-2 contribute
...
**CP-2 INVALIDATION NOTE (final replay, finding AR3-2).** CP-2 was recorded to replace
