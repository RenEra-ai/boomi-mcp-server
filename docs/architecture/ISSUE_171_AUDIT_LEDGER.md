...
| **DC-20** | a file's tracked/ignored/shipped status disagreeing across git, the archive and the build context | what git actually tracks, and what the build context actually uploads | **7** — AR2-4, AR3-1, S1-2, AR4-2, AR5-4, AR6-4, RF-2 | Instance 2 would have broken a GATE rather than a record: `.gitignore` silently excluded the raw logs the checksums listed, visible only on a clean checkout. Instance 4 was the class still OPEN after its structural fix had been CLAIMED — the check existed only as a one-off manual measurement. Structural response, completed at replay #4: the archive scanner compares `SHA256SUMS` against the **git index** as well as the worktree, rejects symlinks, and enumerates with `git ls-files -z` so a whitespace path cannot drop out of the comparison |
...
authorization lives in rows **D-1c/D-2c** under **CP-6**. They are retained as the record of a
