...
| **DC-20** | a file's tracked/ignored/shipped status disagreeing across git, the archive and the build context | what git actually tracks, and what the build context actually uploads | **4** — AR2-4, AR3-1, S1-2, AR4-2 | instance 4 was the class still being OPEN after its own structural fix was claimed: the check existed only as a manual measurement until replay #4. Now mechanised in the archive scanner | instance 2 was the one that would have broken a GATE, not a record: `.gitignore` silently excluded the raw logs the checksums listed, and only a clean checkout could see it. Structural response (completed only at replay #4, per AR4-2): the archive scanner now compares `SHA256SUMS` against the **git index** as well as the worktree. An earlier revision of this cell claimed that verification was already in place when it was a one-off manual measurement |
| **DC-21** | a defect class assigned by resemblance rather than by its own definition | the class definition | **2** — AR4-5, AR3-4a | filing by shape rather than mechanism inflates the resembling class and hides the real one. Response: a class assignment is checked against the class's own definition text at reconciliation |
