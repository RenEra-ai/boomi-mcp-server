...
- The singular top-level `spec.pipeline` is a **single-process** derived view (§5): for a multi-process spec it summarizes the one designated main/entry process, not an aggregate of all processes. Per-process summaries for any additional process are compiler-derived on each process component.
...
**Process scope of the singular derived view (multi-process specs).** `IntegrationSpecV1.components` is a list, so a spec may carry more than one process component (a `wrapper_subprocess` parent plus its child processes, or independently authored processes sharing one materialization plan). The one-root-per-process rule (§3) therefore yields one ProcessIR/CFG **per process**, while the spec envelope exposes only a **single** top-level `pipeline` field (`src/boomi_mcp/models/integration_models.py:90-97`). To keep #139's derivation and its equality/conflict check unambiguous, that top-level `spec.pipeline` view is scoped to exactly **one designated main/entry process** — the same process whose executable `main_process.config.pipeline` it may duplicate today — and the derived-equality-or-`LEGACY_ADAPTER_AUTHORITY_CONFLICT` decision above runs against **that** process's ProcessIR only. Summaries for any additional process are compiler-derived **per process component**, never folded into the singular top-level field; a spec with no single designated main process leaves `spec.pipeline` `None` rather than aggregating. #139 owns implementing this scoping (equivalently, relocating the derived view onto each process component); it must not leave the top-level field as an ambiguous multi-process aggregate.
