...
- **Named adapters** (linear `sync_pipeline`, `flow_sequence`, `wrapper_subprocess`, legacy block configs, archetype/composition recipes) normalize each compatibility input into exactly one ProcessIR root. Adapters never emit XML themselves. (For a *recipe* this one-root rule covers its process-semantic contribution; a #145 structured recipe may additionally author typed component-plan/topology/constraint contributions to their own authorities — §6's typed-contribution note — without widening the process-compiler input.)
...
**Amendment (#146 amendment) — the sanitized behavioural projection is a compiler-derived INSPECTION VIEW.** §6 keeps compiler internals off LLM-facing surfaces, and two of the three capability registries state in their own charters that no MCP tool reaches them. That was measured to be a real cost, not a theoretical one: a caller who violated a body-placement rule was pointed at a repository file no MCP tool can fetch, roughly half the served `$defs` carried no description at all, and the words "sequential" and "parallel" occurred zero times in the served ProcessIR schema. The behavioural facts an LLM needs to author a valid process — that Branch paths run in the authored order, that a database `Send` returns nothing, that retry over an `unverified` action is refused whatever evidence is attached — existed only inside the compiler.

This ADR therefore admits **one named seam**, and bounds it:

- **Output only.** `boomi_mcp.authoring.process_ir_projection` may READ the named registries (`body_capabilities`, `connector_capabilities`, `error_handling`) to derive a read-only contract. Nothing a caller sends re-enters the compiler through it; those registries remain the sole enforcement authority, and the projection can neither mutate nor override them. It is the same category as the derived `spec.pipeline` view of §4 — an inspection view, not a second input surface.
- **Semantic facts, not compiler representations.** The projection publishes what a construct MEANS under a distinct public vocabulary. Where an internal identifier would otherwise cross, it is renamed through a total, injective, parity-pinned map (a Branch outlet is a `branch_path`; replay safety is a `replay_classification`). Capability *state values* cross verbatim — renaming a state would be the drift this contract exists to remove.
- **Nothing compiler-owned crosses.** No CFG edge, semantic or emission model, node id, emitter key, emission-plan wiring, dragpoint, coordinate, layout, XML, credential, or document content. A repository-wide guard scans every served payload for the internal identifiers and fails the build on any of them.
- **Generated or parity-pinned, always attributed.** Every entry is either computed from a named runtime registry or held against one by CI, and records which. The two are not equally strong and the difference is stated rather than blurred: a `generated` entry cannot disagree with its source because it is derived from it, while a `parity_pinned` entry states something prose must say — its identity and its named source are asserted two-way, and its full served text is frozen in a committed snapshot so any edit lands in a reviewable diff. That is a review gate on the wording, not a proof the wording is true; no test derives an English sentence from a registry. Prose may explain a design implication; it is never an independent executable authority. Drift negative controls prove the pin: changing a capability state, a placement row, a replay classification, a validator remediation or a state-visibility fact without updating the contract fails CI.

The projection is served through the existing `get_schema_template` selector registry (`process_ir_authoring`) and summarized in `list_capabilities`; it introduces no tool and no fourth revision. Because the module charters said "no MCP tool, schema, or builder reaches it", each was amended in place to name this exception — an exception recorded only here would be an exception the next reader of those modules would not find.
...
| `RECIPE_*` | Recipe registry resolution, input/contribution validation, patch composition/conflict, constraint evaluation, and determinism | #145 (`…NOT_FOUND`, `…VERSION_UNAVAILABLE`, `…CAPABILITY_GATED`, `…INPUT_INVALID`, `…CONTRIBUTION_INVALID`, `…PATCH_TARGET_NOT_FOUND`, `…PATCH_CONFLICT`, `…CONSTRAINT_FAILED`, `…OUTPUT_NONDETERMINISTIC`) |
...
| `M12_*` | #147 final-gate audit failures (milestone-closure evidence, not per-request authoring diagnostics) | #147 (`…COMPATIBILITY_INCOMPLETE`, `…LEGACY_PATH_REACHABLE`, `…CAPABILITY_DOC_DRIFT`, `…LIVE_SCHEMA_DRIFT`, `…QA_INCOMPLETE`) |

To keep codes stable across shared introducers, each family's name and code constants live in **one shared registry** from the family's first introducer onward; a later introducer **adds** codes to the existing family — it never renames or re-scopes it. #146 surfaces the eight core families through MCP contracts without first-owning any, and additionally owns the `AUTHORING_*` surface family above; the `M12_*` family is #147's own final-gate vocabulary (introduced and consumed by its milestone-signoff audit, never emitted by an authoring request). All ten families are in scope for the #147 registry audit (#147's own acceptance criterion audits code uniqueness across the ProcessIR, topology, recipe, authoring, legacy, build, and verifier families).

Every diagnostic in these families must carry: a **stable code**, the **authored JSON path** it points at, a **safe remediation** hint, and **no secret-bearing values** (see §11). For the `M12_*` final-gate family — which audits the milestone rather than an authored payload — the audited artifact/scope stands in for the authored JSON path; the other three requirements apply unchanged.
