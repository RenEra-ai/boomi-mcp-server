# cap155-e6-put-attested — PROVENANCE

## What this proves

A REST **PUT** executed TWICE against the same live resource, with a counterparty access
...
-- `cap155-e2-put` ran two executions but archived no access log, and the `cap155-e4-*`
...
|---|---|---|---|
| `execution-116a711b-4c5d-4782-b852-9015a3f1b56d-2026.09.01` | `2026-09-01T19:52:30.170564296Z` | `INFO:     192.168.65.1:35414 - "PUT /admin/cdscm/api/v1/clients/0265a8ec-da14-4347-ab5d-fcf7850b4636 HTTP/1.1" 200 OK` | **200 OK** |
| `execution-6541710d-8096-4bac-a651-5b3703d92097-2026.09.01` | `2026-09-01T19:52:47.890963805Z` | `INFO:     192.168.65.1:42791 - "PUT /admin/cdscm/api/v1/clients/0265a8ec-da14-4347-ab5d-fcf7850b4636 HTTP/1.1" 200 OK` | **200 OK** |

Attribution is unambiguous: `2` logged PUT requests
...
The banked PUT operation names the ITEM, so the attribution path and the readback
path coincide: `/admin/cdscm/api/v1/clients/0265a8ec-da14-4347-ab5d-fcf7850b4636`.
...
Two executions, two logged PUT requests, one per execution, both `200 OK`.

* Both executions reported success: ExecutionRecord `COMPLETE`, in=1 out=2.
* The payload really was identical: the PUT body is the GET stage's output document,
  byte-identical across the two runs, with the body source unmoved at all three read points.
* The second identical PUT left the resource's BUSINESS state unchanged (R1 and R2 share a
  business digest) but did NOT leave the full state identical: exactly ONE field differs,
  `modifiedOn`.
* `createdOn` is stable R1..R2, so the replay replaced the existing row rather than
  recreating it.
* POSITIVE CONTROL, same run: the FIRST PUT did change the business digest (R0 -> R1) --
  `status` went `PENDING` -> `ACTIVE` and the seeded `doomedField` was REMOVED, which is the
  property that separates a whole-body replace from a merge.
* NEGATIVE CONTROL, same run: an untouched control resource, referenced by no operation
  component, is byte-identical at R0, R1 and R2.

So `rest.put` on this counterparty is **conditionally idempotent**: the business state
...
GenericConnectorRecord rows are `SUCCESS`/`SUCCESS` with the same `size`, and the access log
shows `200 OK` for both. The ONLY channel that reveals the replay is the counterparty
READBACK, via `modifiedOn` -- the same result the archived PATCH capture reached. A retry
verdict for this verb therefore cannot be derived from execution records alone.

## Staged readbacks

| point | target HTTP | target business digest | control HTTP | control business digest |
|---|---|---|---|---|
| R0_before | 200 | `32f27970853c` | 200 | `60dbd903bf23` |
| R1_between | 200 | `28e608199eec` | 200 | `60dbd903bf23` |
| R2_after | 200 | `28e608199eec` | 200 | `60dbd903bf23` |
...
* `full_state_identical_R1_R2`: `False`
...
* `component_op_src.xml` <- `cap155-e2-put/source_operation_component.xml` (banked sha256 `279806a7551e9907…`), verb `GET`, path bytes unedited: `True`
* `component_op_tgt.xml` <- `cap155-e2-put/operation_component.xml` (banked sha256 `45f68a3df1b8a595…`), verb `PUT`, path bytes unedited: `True`
...
* operation/connection remap: `{"03ff92d3-46db-4b45-9a52-ccf6de6613ca": "f759bc0c-dd38-49ab-b101-4cbface65a54", "9f2e2ad7-518b-4485-9ab9-e0bbbde75ae2": "24ec2d04-e8d6-484d-a23a-4c1386143807", "c4281346-83e9-4026-856d-ede718ec68a0": "2fe488e4-3169-4529-9515-d854570c8ffc"}`
* send `actionType`: `{"from": "PATCH", "to": "PUT"}`
