...
| Current acceptance | `process_kind="wrapper_subprocess"` + a `process_calls` list of EXACTLY ONE entry (one of `subprocess_ref="$ref:KEY"` / `process_id`); optional `process_extensions`, `description`. **#175:** a multi-call list and `return_documents.enabled=true` are now typed refusals — a Process Call is a path terminal, so a parent may make one call and ends there. Both forms are transferred to #176, which owns binding a returning child's return paths. |
