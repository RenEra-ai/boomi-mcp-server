...
| Current acceptance | `process_kind="wrapper_subprocess"` + a `process_calls` list of EXACTLY ONE entry (one of `subprocess_ref="$ref:KEY"` / `process_id`); optional `process_extensions`, `description`, and `return_documents` in its ACCEPTED form only (`enabled=false`, or the block absent). **#175:** a multi-call list and `return_documents.enabled=true` are now typed refusals — a Process Call is a path terminal, so a parent may make one call and ends there. Both forms are transferred to #176, which owns binding a returning child's return paths. |
| Public / executable | Public (schema docs `meta_tools.py:7838-7937`); executable (`start → processcall`, the call TERMINATING the path — no trailing stop or return, #175) |
...
|---|---|---|---|---|---|---|---|
| `wrapper_subprocess` | `wrapper_subprocess.py` | exactly one process_call, TERMINAL (no Stop, no Return Documents — #175); `wait`/`abort_on_error`/`label` | adapter/compile/emit defect → `PROCESS_XML_VALIDATION_FAILED`; `PROCESS_REF_MISSING`/`PROCESS_EXTENSIONS_INVALID` totality guards preserved (precede the adapter) | `processcall_standalone_parent.xml` | inside `emit_process` (`verify_process_graph`) | required | **canonical** |
...
| `src/boomi_mcp/compiler/process_ir/body_capabilities.py:_walk_body` | Rechecks slot allowlists and connector/ProcessCall path mixing. | Body, scope, catch, nesting codes above | Sound for admitted bodies; fail-fast. | port-unchanged | `tests/test_process_ir_rich_control_bodies.py::test_the_connector_above_call_direction_is_still_live_and_enforced` | None. |
...
| `src/boomi_mcp/models/process_ir.py:BranchLegV1._leg_rules` | Cache ordering, ProcessCall terminal form (#175), nonempty Stop leg. | `PROCESS_IR_SCHEMA_INVALID_CARDINALITY`, `PROCESS_IR_CAPABILITY_NODE_NOT_ALLOWED_IN_BODY` | Sound for V1 grammar. | port-unchanged | `tests/test_process_ir_models.py::test_process_call_is_a_branch_leg_terminal_not_a_step` | None. |
| `src/boomi_mcp/models/process_ir.py:DecisionTrueArmV1._arm_rules` | Cache ordering, ProcessCall terminal form (#175), nonempty Stop. | Same two codes | Sound for V1 true arm. | port-unchanged | `tests/test_process_ir_rich_control_bodies.py::test_a_decision_arm_admits_exactly_one_terminal_process_call` | None. |
...
| `src/boomi_mcp/models/process_ir.py:_check_process_call_terminal_form` | ProcessCall is a body TERMINAL with an empty step prefix (#175 amended #141's Stop-terminal rule); connector mixing still gated. | `PROCESS_IR_CAPABILITY_NODE_NOT_ALLOWED_IN_BODY` | Sound for verified mode. | port-unchanged | `tests/test_process_ir_models.py::test_process_call_is_a_branch_leg_terminal_not_a_step` | None. |
...
| `src/boomi_mcp/categories/components/builders/process_flow_builder.py:_sync_pipeline_is_canonical` | Routes every lowered pipeline to the adapter; #158 made the listener entry representable, so the legacy fallback behind it is unreached. | None | Sound; the listener capability gap closed in #158. | adapter-only-compat | `tests/test_sync_pipeline_adapter_cutover.py::test_listener_chain_reaches_the_adapter` | #160 deletes the gate with its unreached fallback. |
...
| `src/boomi_mcp/compiler/process_ir/legacy_adapters/wrapper_subprocess.py:adapt_wrapper_subprocess` | Validated single-call conversion (#175: exactly one call, no appended terminal); multi-call and enabled Return Documents refused. | `LEGACY_ADAPTER_SEMANTIC_LOSS`; internal schema errors | Sound syntax; incomplete without child summary. | adapter-only-compat | `tests/test_legacy_process_ir_adapters.py::test_wrapper_requirements_are_process_typed` | `LEGACY_ADAPTER_EXEMPTION_SUBPROCESS_SUMMARY`; summary coverage gate. |
...
- `tests/patterns/test_recipe_preset_parity.py::test_l4_legacy_cache_arm_passes_strictly_once_cache_identity_is_canonical`
...
|---|---|---|---|---|---|---|---|
| LG-aee1eb03 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_emitters/legacy.py | _emit_setproperties | 3 | 545 | #160 | delete with the legacy renderer |
| LG-420f1954 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_emitters/legacy.py | _emit_setproperties_step | 3 | 485 | #160 | delete with the legacy renderer |
| LG-a8efa464 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | <module> | 29 | 82 | #160 | delete the legacy semantic shell |
| LG-e3bb78f3 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_branch_shapes | 1 | 3417 | #160 | delete the legacy semantic shell |
| LG-09c051cf | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_catch_leg | 5 | 4577 | #160 | delete the legacy semantic shell |
| LG-43cb4e79 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_connector_scoped_try_catch_shapes | 3 | 4692 | #160 | delete the legacy semantic shell |
| LG-51f10b57 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_decision_shapes | 1 | 3494 | #160 | delete the legacy semantic shell |
| LG-e1ff295b | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_flow_shape | 14 | 3297 | #160 | delete the legacy semantic shell |
| LG-e9731d65 | legacy_emitter | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_try_catch_shapes | 2 | 4411 | #160 | delete the legacy semantic shell |
| LG-9f90fa64 | legacy_semantic_validation | src/boomi_mcp/categories/integration_builder.py | _process_component_preflight | 1 | 6588 | #160 | delete with the legacy semantic shell |
| LG-599ecc0d | legacy_semantic_validation | src/boomi_mcp/categories/integration_builder.py | _process_ir_semantic_error | 1 | 6614 | #160 | delete with the legacy semantic shell |
| LG-48cc5eb8 | legacy_transitive_call | server.py | analyze_component | 1 | 1919 | #160 | follow the callee's row; this is an edge, not a site |
| LG-853101d8 | legacy_transitive_call | server.py | apply_component_edit | 1 | 2077 | #160 | follow the callee's row; this is an edge, not a site |
| LG-b6bb8a5c | legacy_transitive_call | server.py | build_integration | 1 | 2524 | #160 | follow the callee's row; this is an edge, not a site |
| LG-489d0ece | legacy_transitive_call | server.py | compose_archetypes | 1 | 2647 | #160 | follow the callee's row; this is an edge, not a site |
| LG-a389d52e | legacy_transitive_call | server.py | discover_odata_metadata | 1 | 3067 | #160 | follow the callee's row; this is an edge, not a site |
| LG-1a0b93dd | legacy_transitive_call | server.py | discover_openapi_spec | 1 | 2966 | #160 | follow the callee's row; this is an edge, not a site |
| LG-97044988 | legacy_transitive_call | server.py | discover_soap_wsdl | 1 | 3018 | #160 | follow the callee's row; this is an edge, not a site |
| LG-f5622f58 | legacy_transitive_call | server.py | invoke_boomi_api | 1 | 3456 | #160 | follow the callee's row; this is an edge, not a site |
| LG-963c3aaf | legacy_transitive_call | server.py | manage_component | 1 | 1818 | #160 | follow the callee's row; this is an edge, not a site |
| LG-2976fe39 | legacy_transitive_call | server.py | manage_connector | 1 | 2181 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-b7cd58ae | legacy_transitive_call | src/boomi_mcp/authoring/workflow.py | compile_authoring_request_v1 | 1 | 3210 | #160 | follow the callee's row; this is an edge, not a site |
| LG-14572d6c | legacy_transitive_call | src/boomi_mcp/authoring/workflow.py | plan_authoring_request_v1 | 2 | 2578 | #160 | follow the callee's row; this is an edge, not a site |
| LG-f57fd778 | legacy_transitive_call | src/boomi_mcp/authoring/workflow.py | preflight_typed_apply_v1 | 1 | 3451 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-020c9559 | legacy_transitive_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_connector_scoped_try_catch_shapes | 7 | 4684 | #160 | follow the callee's row; this is an edge, not a site |
| LG-2296966e | legacy_transitive_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_decision_leg | 1 | 3446 | #160 | follow the callee's row; this is an edge, not a site |
| LG-8375bff7 | legacy_transitive_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_decision_shapes | 3 | 3480 | #160 | follow the callee's row; this is an edge, not a site |
| LG-2dd324b1 | legacy_transitive_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_flow_shape | 2 | 3316 | #160 | follow the callee's row; this is an edge, not a site |
| LG-0b2a5bd8 | legacy_transitive_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_linear_shapes | 1 | 3375 | #160 | follow the callee's row; this is an edge, not a site |
| LG-3ac90371 | legacy_transitive_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _emit_try_catch_shapes | 2 | 4405 | #160 | follow the callee's row; this is an edge, not a site |
| LG-225b9aa0 | legacy_transitive_call | src/boomi_mcp/categories/components/connectors.py | create_connector | 3 | 389 | #160 | follow the callee's row; this is an edge, not a site |
| LG-b0e06309 | legacy_transitive_call | src/boomi_mcp/categories/components/connectors.py | manage_connector_action | 2 | 734 | #160 | follow the callee's row; this is an edge, not a site |
| LG-6f781717 | legacy_transitive_call | src/boomi_mcp/categories/components/manage_component.py | clone_component | 1 | 400 | #160 | follow the callee's row; this is an edge, not a site |
| LG-9afe2de9 | legacy_transitive_call | src/boomi_mcp/categories/components/manage_component.py | create_component | 6 | 82 | #160 | follow the callee's row; this is an edge, not a site |
| LG-2cc27d39 | legacy_transitive_call | src/boomi_mcp/categories/components/manage_component.py | manage_component_action | 3 | 483 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-dcad5d33 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _account_listen_operation_facts | 1 | 829 | #160 | follow the callee's row; this is an edge, not a site |
| LG-c02d94f3 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _listener_operation_ref_from_process | 1 | 1283 | #160 | follow the callee's row; this is an edge, not a site |
| LG-0a50fc28 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _resolve_listener_metadata | 1 | 1432 | #160 | follow the callee's row; this is an edge, not a site |
| LG-cde80696 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _reused_listener_entry_mismatch | 1 | 1025 | #160 | follow the callee's row; this is an edge, not a site |
| LG-90edd5fc | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _root_entry | 1 | 753 | #160 | follow the callee's row; this is an edge, not a site |
| LG-205964b1 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _run_listener_verify_stage | 2 | 2344 | #160 | follow the callee's row; this is an edge, not a site |
| LG-8e5effa6 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _standalone_entry_refusal | 1 | 974 | #160 | follow the callee's row; this is an edge, not a site |
| LG-65019d95 | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | _target_listener_operation_id | 1 | 800 | #160 | follow the callee's row; this is an edge, not a site |
| LG-ab50f45d | legacy_transitive_call | src/boomi_mcp/categories/deployment/orchestration.py | orchestrate_deploy_action | 6 | 5379 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-3cf79d6e | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _apply_plan | 4 | 9768 | #160 | follow the callee's row; this is an edge, not a site |
| LG-3a7213a4 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _authored_process_validation_error | 1 | 5847 | #160 | follow the callee's row; this is an edge, not a site |
| LG-86e93983 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _build_plan | 5 | 6703 | #160 | follow the callee's row; this is an edge, not a site |
| LG-8c22801a | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _check_api_service_route_dependencies | 1 | 2195 | #160 | follow the callee's row; this is an edge, not a site |
| LG-ab4b2334 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _compile_authoring | 1 | 12320 | #160 | follow the callee's row; this is an edge, not a site |
| LG-01c3922d | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _execute_canonical_process | 2 | 9199 | #160 | follow the callee's row; this is an edge, not a site |
| LG-f16d0493 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _execute_component | 23 | 3818 | #160 | follow the callee's row; this is an edge, not a site |
| LG-308b6e78 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _plan_authoring | 1 | 12258 | #160 | follow the callee's row; this is an edge, not a site |
| LG-f1e10478 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _process_component_preflight | 2 | 6458 | #160 | follow the callee's row; this is an edge, not a site |
| LG-0a17dc6a | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _process_config_has_wss_listen | 1 | 2069 | #160 | follow the callee's row; this is an edge, not a site |
| LG-4ef294c2 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | _resolve_preservation_policy | 1 | 1408 | #160 | follow the callee's row; this is an edge, not a site |
| LG-1f766152 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | build_integration_action | 4 | 12648 | #160 | follow the callee's row; this is an edge, not a site |
| LG-8796f643 | legacy_transitive_call | src/boomi_mcp/categories/integration_builder.py | build_structured_update_xml | 1 | 3424 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-dec16977 | legacy_transitive_call | src/boomi_mcp/compiler/process_ir/legacy_adapters/authority.py | evaluate_pipeline_authority | 2 | 396 | #160 | follow the callee's row; this is an edge, not a site |
| LG-8c220ac8 | legacy_transitive_call | src/boomi_mcp/patterns/archetypes/api_to_api_sync.py | ApiToApiSyncArchetype.emit_spec | 1 | 764 | #160 | follow the callee's row; this is an edge, not a site |
| LG-492bfbd5 | legacy_transitive_call | src/boomi_mcp/patterns/archetypes/api_to_database_sync.py | ApiToDatabaseSyncArchetype.emit_spec | 1 | 847 | #160 | follow the callee's row; this is an edge, not a site |
| LG-492d9518 | legacy_transitive_call | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | DatabaseToApiSyncArchetype.emit_spec | 1 | 2460 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-0ea86058 | registry_lookup | src/boomi_mcp/authoring/process_entry.py | legacy_config_entry | 2 | 107 | #160 | delete with the legacy registry |
| LG-a6b08ee4 | registry_lookup | src/boomi_mcp/categories/components/builders/process_flow_builder.py | get_process_flow_builder | 2 | 6175 | #160 | delete the legacy semantic shell |
| LG-e131eb91 | registry_lookup | src/boomi_mcp/categories/integration_builder.py | _execute_component | 3 | 3818 | #153 | replace with canonical ProcessIR materialization/apply |
| LG-85b86f7b | registry_lookup | src/boomi_mcp/categories/integration_builder.py | _process_component_preflight | 3 | 6354 | #153 | replace with canonical ProcessIR materialization/apply |
| LG-f76e62cd | registry_lookup | src/boomi_mcp/categories/integration_builder.py | _resolve_preservation_policy | 1 | 1408 | #153 | replace with canonical ProcessIR materialization/apply |
| LG-4e882820 | registry_lookup | src/boomi_mcp/categories/integration_builder.py | build_structured_update_xml | 3 | 3420 | #153 | replace with canonical ProcessIR materialization/apply |
| LG-e4a0a304 | renderer_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | SyncPipelineBuilder.build | 1 | 6070 | #160 | delete the legacy semantic shell |
| LG-088efd1d | renderer_call | src/boomi_mcp/categories/components/builders/process_flow_builder.py | SyncPipelineBuilder.validate_config | 1 | 6014 | #160 | delete the legacy semantic shell |
| LG-9532f2cf | renderer_call | src/boomi_mcp/categories/integration_builder.py | _execute_component | 1 | 3847 | #153 | replace with canonical ProcessIR materialization/apply |
| LG-e5f61bc6 | renderer_call | src/boomi_mcp/categories/integration_builder.py | _process_component_preflight | 2 | 6482 | #153 | replace with canonical ProcessIR materialization/apply |
| LG-7c23f2fd | renderer_call | src/boomi_mcp/categories/integration_builder.py | build_structured_update_xml | 1 | 3437 | #153 | replace with canonical ProcessIR materialization/apply |
...
| LG-f973cf28 | renderer_call | src/boomi_mcp/compiler/process_ir/legacy_adapters/authority.py | _core_from_submitted_config | 4 | 293 | #151 | re-home onto the neutral extraction |
| LG-819f78b4 | renderer_call | src/boomi_mcp/compiler/process_ir/legacy_adapters/sync_pipeline.py | adapt_sync_pipeline_config | 1 | 540 | #151 | re-home onto the neutral extraction |
...
| LG-3bd11d29 | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | SyncPipelineBuilder | 1 | 5318 | #160 | delete the legacy semantic shell |
| LG-42425307 | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | SyncPipelineBuilder.lower_config | 1 | 5508 | #160 | delete the legacy semantic shell |
| LG-513dee34 | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | SyncPipelineBuilder.validate_config | 6 | 5995 | #160 | delete the legacy semantic shell |
| LG-65e5dfbc | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | WrapperSubprocessBuilder | 1 | 4898 | #160 | delete the legacy semantic shell |
| LG-69d179d0 | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | WrapperSubprocessBuilder.validate_config | 6 | 4951 | #160 | delete the legacy semantic shell |
| LG-67ac22ad | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _reliability_requests_try_catch | 1 | 2165 | #160 | delete the legacy semantic shell |
| LG-b4eb25cd | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | _source_dynamic_path_connector_scope_error | 1 | 1722 | #160 | delete the legacy semantic shell |
| LG-943d38e3 | unclassified_reference | src/boomi_mcp/categories/components/builders/process_flow_builder.py | get_process_flow_builder | 3 | 6171 | #160 | delete the legacy semantic shell |
| LG-b1b42a81 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _apply_plan | 1 | 11207 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-98073b97 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _authored_process_validation_error | 4 | 5840 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-cd5de55e | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _bracketed_naming_warning | 2 | 5284 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-32fcd45b | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _build_plan | 5 | 6999 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-994f0a04 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _execute_component | 7 | 3814 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-66c9297f | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _process_component_preflight | 13 | 6300 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-79c56926 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _process_ir_semantic_error | 2 | 6594 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-61ad1e75 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _process_models_error_handling | 5 | 4569 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-378bc32b | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _resolve_preservation_policy | 5 | 1399 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-81ba94b9 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _scan_top_level_pipeline_secrets | 1 | 5755 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-a6339083 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _synthesize_wrapper_subprocess_edges | 3 | 578 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-df05d9d2 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | _synthesize_wrapper_subprocess_extensions | 3 | 742 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-2ae43575 | unclassified_reference | src/boomi_mcp/categories/integration_builder.py | build_structured_update_xml | 7 | 3406 | #160 | residue: a watched name mentioned in a shape the census does not classify |
...
| LG-feb448d7 | unclassified_reference | src/boomi_mcp/categories/meta_tools.py | <module> | 5 | 8080 | #160 | retract the served legacy guidance / guard the raw route |
| LG-de5e36cb | unclassified_reference | src/boomi_mcp/compiler/process_ir/emitter_registry.py | <module> | 17 | 738 | #160 | residue: a watched name mentioned in a shape the census does not classify |
...
| LG-7426e6bc | unclassified_reference | src/boomi_mcp/compiler/process_ir/legacy_adapters/sync_pipeline.py | <module> | 1 | 63 | #151 | re-home onto the neutral extraction |
| LG-ca32b52e | unclassified_reference | src/boomi_mcp/compiler/process_ir/legacy_adapters/wrapper_subprocess.py | <module> | 2 | 32 | #151 | re-home onto the neutral extraction |
...
| LG-f6ce64dd | unclassified_reference | src/boomi_mcp/compiler/process_ir/semantic_validation/legacy_bridge.py | _adapter_for_config | 2 | 44 | #151 | re-home onto the neutral extraction |
...
| LG-cf15d311 | unclassified_reference | src/boomi_mcp/models/_process_ir_compat.py | <module> | 4 | 217 | #159 | migrate the compatibility codec, then delete |
| LG-f26f5a1c | unclassified_reference | src/boomi_mcp/models/_process_ir_compat.py | legacy_flow_sequence_to_ir | 5 | 679 | #159 | migrate the compatibility codec, then delete |
| LG-e24ae58a | unclassified_reference | src/boomi_mcp/patterns/archetype_assembly.py | _build_rest_send_params | 7 | 551 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-4291f7db | unclassified_reference | src/boomi_mcp/patterns/archetype_parameters.py | ApiTarget | 1 | 1566 | #160 | residue: a watched name mentioned in a shape the census does not classify |
...
| LG-fd0fb833 | unclassified_reference | src/boomi_mcp/patterns/archetypes/api_to_api_sync.py | ApiToApiSyncArchetype | 2 | 453 | #159 | migrate the archetype to canonical ProcessIR |
| LG-c23b1925 | unclassified_reference | src/boomi_mcp/patterns/archetypes/api_to_api_sync.py | ApiToApiSyncArchetype.emit_spec | 4 | 591 | #159 | migrate the archetype to canonical ProcessIR |
| LG-c65a6048 | unclassified_reference | src/boomi_mcp/patterns/archetypes/api_to_api_sync.py | _build_pipeline_dict | 1 | 281 | #159 | migrate the archetype to canonical ProcessIR |
| LG-a34a1a82 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | DatabaseToApiSyncArchetype | 2 | 2019 | #159 | migrate the archetype to canonical ProcessIR |
| LG-b9ecf83d | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | DatabaseToApiSyncArchetype.emit_spec | 3 | 2211 | #159 | migrate the archetype to canonical ProcessIR |
| LG-12c696f9 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | DatabaseToApiSyncParameters._enforce_watermark_consistency | 1 | 738 | #159 | migrate the archetype to canonical ProcessIR |
| LG-a209f251 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | DatabaseToApiSyncParameters._validate_transform_refs | 7 | 821 | #159 | migrate the archetype to canonical ProcessIR |
| LG-176df664 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | _build_dynamic_path | 9 | 1336 | #159 | migrate the archetype to canonical ProcessIR |
| LG-f58c90b7 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | _build_operational_intent | 4 | 1735 | #159 | migrate the archetype to canonical ProcessIR |
| LG-2b21dfd2 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | _build_rest_send_params | 7 | 1134 | #159 | migrate the archetype to canonical ProcessIR |
| LG-05070148 | unclassified_reference | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | _build_sync_pipeline_adapter_config | 3 | 1432 | #159 | migrate the archetype to canonical ProcessIR |
| LG-e4d03e77 | unclassified_reference | src/boomi_mcp/patterns/archetypes/http_listener_to_rest.py | HttpListenerToRestArchetype | 1 | 345 | #159 | migrate the archetype to canonical ProcessIR |
| LG-91586a36 | unclassified_reference | src/boomi_mcp/patterns/archetypes/http_listener_to_rest.py | HttpListenerToRestArchetype.emit_spec | 2 | 396 | #159 | migrate the archetype to canonical ProcessIR |
| LG-d6d3360b | unclassified_reference | src/boomi_mcp/patterns/composition.py | _fanout_target_params | 5 | 717 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-48a0cf76 | unclassified_reference | src/boomi_mcp/patterns/composition.py | compose_archetypes | 5 | 992 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-1dd09f43 | unclassified_reference | src/boomi_mcp/patterns/composition.py | validate_composition | 2 | 606 | #160 | residue: a watched name mentioned in a shape the census does not classify |
...
| LG-dd69301a | unclassified_reference | src/boomi_mcp/patterns/primitives/document_cache_put.py | DocumentCachePutPrimitive | 1 | 90 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-7063ef77 | unclassified_reference | src/boomi_mcp/patterns/primitives/document_cache_remove.py | DocumentCacheRemovePrimitive | 1 | 128 | #160 | residue: a watched name mentioned in a shape the census does not classify |
| LG-6a39c73a | unclassified_reference | src/boomi_mcp/patterns/primitives/document_cache_retrieve.py | DocumentCacheRetrievePrimitive | 1 | 124 | #160 | residue: a watched name mentioned in a shape the census does not classify |
...
| LG-4cfdfe60 | unclassified_reference | src/boomi_mcp/patterns/primitives/return_documents.py | ReturnDocumentsPrimitive | 1 | 109 | #160 | residue: a watched name mentioned in a shape the census does not classify |
...
| Ledger ID | Census | Path | Symbol | Sites | Baseline line | Owning issue | Disposition |
|---|---|---|---|---|---|---|---|
| LG-a4f2eecb | authoring_boundary | server.py | build_from_archetype | 1 | 2596 | #159 | migrate the boundary to canonical ProcessIR |
| LG-702def56 | authoring_boundary | server.py | build_integration | 1 | 2348 | #159 | migrate the boundary to canonical ProcessIR |
| LG-e2d1badd | authoring_boundary | server.py | compose_archetypes | 1 | 2624 | #159 | migrate the boundary to canonical ProcessIR |
| LG-4b265c33 | authoring_boundary | server.py | import_integration_draft | 1 | 2860 | #159 | migrate the boundary to canonical ProcessIR |
...
| LG-18b251bf | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _authored_process_validation_error | 2 | 5841 | #160 | delete with the legacy consumer |
| LG-0f6e121e | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _bracketed_naming_warning | 2 | 5284 | #160 | delete with the legacy consumer |
| LG-adf3120a | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _build_plan | 2 | 7000 | #160 | delete with the legacy consumer |
| LG-79a2008e | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _execute_component | 2 | 3815 | #160 | delete with the legacy consumer |
| LG-7e8d39d2 | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _process_models_error_handling | 2 | 4570 | #160 | delete with the legacy consumer |
| LG-f633058d | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _resolve_preservation_policy | 2 | 1400 | #160 | delete with the legacy consumer |
| LG-331a98ac | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _synthesize_wrapper_subprocess_edges | 2 | 578 | #160 | delete with the legacy consumer |
| LG-76727309 | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | _synthesize_wrapper_subprocess_extensions | 2 | 742 | #160 | delete with the legacy consumer |
| LG-0e50752e | process_kind_consumer | src/boomi_mcp/categories/integration_builder.py | build_structured_update_xml | 2 | 3407 | #160 | delete with the legacy consumer |
...
| LG-af904e61 | process_kind_consumer | src/boomi_mcp/models/_process_ir_compat.py | legacy_flow_sequence_to_ir | 2 | 679 | #159 | migrate the compatibility codec, then delete |
| LG-885d1a16 | process_kind_producer | src/boomi_mcp/categories/components/builders/process_flow_builder.py | SyncPipelineBuilder.lower_config | 1 | 5508 | #160 | delete the legacy semantic shell |
...
| LG-76cde1a2 | process_kind_producer | src/boomi_mcp/patterns/archetypes/api_to_database_sync.py | _build_main_process | 2 | 525 | #159 | migrate the archetype to canonical ProcessIR |
| LG-0bf8b950 | process_kind_producer | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | _build_main_process | 1 | 1605 | #159 | migrate the archetype to canonical ProcessIR |
| LG-22b7f2cc | process_kind_producer | src/boomi_mcp/patterns/archetypes/database_to_api_sync.py | _build_sync_pipeline_adapter_config | 1 | 1434 | #159 | migrate the archetype to canonical ProcessIR |
| LG-78a58f1a | process_kind_producer | src/boomi_mcp/patterns/archetypes/http_listener_to_db.py | HttpListenerToDbArchetype.emit_spec | 1 | 1173 | #159 | migrate the archetype to canonical ProcessIR |
| LG-2749063b | process_kind_producer | src/boomi_mcp/patterns/archetypes/http_listener_to_db.py | _build_listener_main_process | 2 | 810 | #159 | migrate the archetype to canonical ProcessIR |
| LG-1e119c4f | process_kind_producer | src/boomi_mcp/patterns/archetypes/http_listener_to_rest.py | HttpListenerToRestArchetype.emit_spec | 1 | 555 | #159 | migrate the archetype to canonical ProcessIR |
...
|---|---|---|---|---|---|
| WRT-canonical-process-materialization | src/boomi_mcp/categories/integration_builder.py::_execute_canonical_process | canonical_process_materialization | #153: compiles the authored ProcessIR with placeholder ids, rebinds them to the ids ordered apply has published, recompiles, emits, and wraps the result with the neutral ProcessComponentMaterializer. The caller supplies semantics, never XML. | #153 | unchanged, and it is what the retraction leaves behind: a process root reaches the platform only through a compiled artifact whose plan fingerprint and apply-time mutation attestation are both recorded. |
...
| WRT-external-transport-oauth-callback | server.py::web_callback | external_transport | OAuth callback posts to the identity provider's token endpoint via a hand-rolled HTTP client. Never targets the Boomi Component API. | #160 | unchanged — assert the target host/path is the configured token endpoint, never /Component. |
| WRT-external-transport-listener-probe | src/boomi_mcp/categories/deployment/orchestration.py::_listener_probe | external_transport | Probes a deployed listener's own URL with `urllib.request.urlopen` to confirm it is serving. Not a platform API call. | #160 | unchanged — the URL comes from the listener's endpoint, never from a Component route. |
| WRT-external-transport-marketplace | src/boomi_mcp/categories/marketplace.py::search_marketplace_recipes_action | external_transport | Public, unauthenticated Marketplace GraphQL query over httpx. Carries no platform credentials and has no install/write path. | #160 | unchanged — assert the endpoint is the Marketplace GraphQL host and no component XML is submitted. |
| WRT-external-transport-schema-discovery | src/boomi_mcp/categories/schema_discovery.py::_fetch | external_transport | Fetches a caller-supplied OpenAPI/WSDL/OData document over httpx under the SSRF/redirect guards. Reads a third-party URL, never the platform. | #160 | unchanged — the SSRF guard already forbids platform hosts; assert no /Component target is reachable. |
...
| Ledger ID | Census | Path | Symbol | Sites | Baseline line | Owning issue | Disposition |
|---|---|---|---|---|---|---|---|
| LG-4e092c94 | component_xml_write | src/boomi_mcp/categories/components/_shared.py | _create_component_raw | 1 | 497 | #160 | WRT-shared-raw-create-sink: the shared process-content classifier is enforced at or before this sink for every caller. |
| LG-c7a85e57 | component_xml_write | src/boomi_mcp/categories/components/_shared.py | _update_component_xml | 1 | 529 | #160 | WRT-shared-dormant-writer: sits behind the two-sided guard, or is deleted. |
| LG-08665fc8 | component_xml_write | src/boomi_mcp/categories/components/analyze_component.py | merge_versions | 1 | 707 | #160 | WRT-analyze-component-merge: REJECT when the target is a process OR the source/merged root is a process — the merged root IS the source root, so a target-only check never sees a process-root source over a non-process target. |
| LG-b091f264 | component_xml_write | src/boomi_mcp/categories/components/connectors.py | create_connector | 3 | 389 | #160 | WRT-connector-typed-build: unchanged — the content guard never matches a connector root. · WRT-manage-connector-create: content-based refusal on a process XML root. |
| LG-91d77b6a | component_xml_write | src/boomi_mcp/categories/components/connectors.py | update_connector | 2 | 567 | #160 | WRT-manage-connector-metadata: semantic-body identity under the shared projection. · WRT-manage-connector-update: two-sided payload-OR-live-target refusal. |
| LG-eb7e27c9 | component_xml_write | src/boomi_mcp/categories/components/manage_component.py | clone_component | 1 | 400 | #160 | WRT-manage-component-clone: process-typed clone REJECTED; non-process clone preserved. |
| LG-87f4dbe2 | component_xml_write | src/boomi_mcp/categories/components/manage_component.py | create_component | 6 | 82 | #160 | WRT-manage-component-create: create with a process XML root is REJECTED by the shared process-content classifier, whatever the declared type. · WRT-manage-component-typed-create: unchanged — the content guard never matches these roots. |
| LG-f1dd2559 | component_xml_write | src/boomi_mcp/categories/components/manage_component.py | manage_component_action | 2 | 483 | #160 | WRT-manage-component-dispatch: the shared process-content classifier runs at the dispatcher, before either arm. |
| LG-56fa02b3 | component_xml_write | src/boomi_mcp/categories/components/manage_component.py | update_component | 3 | 248 | #160 | WRT-manage-component-metadata: semantic-body identity under the single shared projection. · WRT-manage-component-update: two-sided check — payload root `process` OR live target type `process` refuses; lookup/parse failure fails closed. |
...
| LG-5f228a61 | component_xml_write | src/boomi_mcp/categories/integration_builder.py | _apply_structured_update | 1 | 3308 | #153 | WRT-build-integration-preservation-merge: preserved, re-homed onto the canonical apply path. |
| LG-d2124aa7 | component_xml_write | src/boomi_mcp/categories/integration_builder.py | _execute_canonical_process | 1 | 9255 | #153 | WRT-canonical-process-materialization: unchanged, and it is what the retraction leaves behind: a process root reaches the platform only through a compiled artifact whose plan fingerprint and apply-time mutation attestation are both recorded. |
| LG-c45135b8 | component_xml_write | src/boomi_mcp/categories/integration_builder.py | _execute_component | 10 | 3861 | #153/#160 | WRT-build-integration-generic: content guard at BOTH plan and apply boundaries, placed BEFORE all type dispatch (the process and connector arms dispatch before the fall-through ever runs); mutation-tested with an unknown/future declared type. · WRT-build-integration-structured-process: replaced by canonical ProcessIR materialization/apply. · WRT-build-integration-typed-nonprocess: unchanged — the content guard never matches these roots. |
| LG-1d129c81 | component_xml_write | src/boomi_mcp/categories/meta_tools.py | invoke_api | 1 | 5924 | #160 | WRT-raw-api-component: ONE canonical endpoint parser feeds classification, ID extraction AND transport; the reserved literal `bulk` is matched BEFORE the `<id>` arm and is never a componentId; every update-shaped call runs the two-sided process check. |
...
| LG-efa0b8f0 | http_client_call | server.py | web_callback | 1 | 5427 | #160 | WRT-external-transport-oauth-callback: unchanged — assert the target host/path is the configured token endpoint, never /Component. |
| LG-7709911a | http_client_call | src/boomi_mcp/categories/deployment/orchestration.py | _listener_probe | 1 | 1644 | #160 | WRT-external-transport-listener-probe: unchanged — the URL comes from the listener's endpoint, never from a Component route. |
...
| LG-c3bf6d69 | raw_api_invoker | server.py | invoke_boomi_api | 1 | 3456 | #160 | WRT-raw-api-component: ONE canonical endpoint parser feeds classification, ID extraction AND transport; the reserved literal `bulk` is matched BEFORE the `<id>` arm and is never a componentId; every update-shaped call runs the two-sided process check. |
...
|---|---|---|
| #151 | re-home onto the neutral extraction | 15 |
| #153 | WRT-build-integration-preservation-merge: preserved, re-homed onto the canonical apply path. | 1 |
| #153 | WRT-canonical-process-materialization: unchanged, and it is what the retraction leaves behind: a process root reaches the platform only through a compiled artifact whose plan fingerprint and apply-time mutation attestation are both recorded. | 1 |
...
| #160 | WRT-external-transport-listener-probe: unchanged — the URL comes from the listener's endpoint, never from a Component route. | 1 |
| #160 | WRT-external-transport-marketplace: unchanged — assert the endpoint is the Marketplace GraphQL host and no component XML is submitted. | 1 |
| #160 | WRT-external-transport-oauth-callback: unchanged — assert the target host/path is the configured token endpoint, never /Component. | 1 |
| #160 | WRT-external-transport-schema-discovery: unchanged — the SSRF guard already forbids platform hosts; assert no /Component target is reachable. | 1 |
| #160 | WRT-folders-move-component: semantic-body identity under the shared projection. | 1 |
| #160 | WRT-manage-component-clone: process-typed clone REJECTED; non-process clone preserved. | 1 |
| #160 | WRT-manage-component-create: create with a process XML root is REJECTED by the shared process-content classifier, whatever the declared type. · WRT-manage-component-typed-create: unchanged — the content guard never matches these roots. | 1 |
| #160 | WRT-manage-component-dispatch: the shared process-content classifier runs at the dispatcher, before either arm. | 1 |
| #160 | WRT-manage-component-metadata: semantic-body identity under the single shared projection. · WRT-manage-component-update: two-sided check — payload root `process` OR live target type `process` refuses; lookup/parse failure fails closed. | 1 |
| #160 | WRT-manage-connector-metadata: semantic-body identity under the shared projection. · WRT-manage-connector-update: two-sided payload-OR-live-target refusal. | 1 |
| #160 | WRT-raw-api-component: ONE canonical endpoint parser feeds classification, ID extraction AND transport; the reserved literal `bulk` is matched BEFORE the `<id>` arm and is never a componentId; every update-shaped call runs the two-sided process check. | 2 |
| #160 | WRT-safe-edit-metadata: route-sensitive projection — the permitted subset is exactly the requested name/folderId/folderName/immediate description; process BODY edits are REJECTED in favour of canonical ProcessIR apply. | 1 |
...
| #160 | delete the legacy semantic shell | 24 |
| #160 | delete with the legacy consumer | 11 |
| #160 | delete with the legacy registry | 1 |
...
never deleted. That is why the freeze snapshots the **served values** — 108 artifacts collected
...
| **Mutation:** a module-qualified wrapper is reported | `tests/test_issue_149_legacy_reachability_freeze.py::test_a_module_qualified_wrapper_is_reported` |
| **Precision:** an unrelated function sharing a bearing name is not reported | `tests/test_issue_149_legacy_reachability_freeze.py::test_an_unrelated_function_sharing_a_bearing_name_is_not_reported` |
| **Mutation:** a one-argument `setdefault` is still a producer | `tests/test_issue_149_legacy_reachability_freeze.py::test_a_one_argument_setdefault_is_still_a_producer` |
| The legacy process-protocol templates are frozen | `tests/test_issue_149_legacy_reachability_freeze.py::test_the_legacy_process_protocol_templates_are_frozen` |
| §11 gives #160 one instruction per site (no contradictions) | `tests/test_issue_149_legacy_reachability_freeze.py::test_no_ledger_row_contradicts_its_write_route` |
| **Mutation:** a barrel-re-export wrapper is reported | `tests/test_issue_149_legacy_reachability_freeze.py::test_a_wrapper_importing_through_a_barrel_reexport_is_reported` |
| **Guard the guard:** the barrel-re-export edges exist at HEAD | `tests/test_issue_149_legacy_reachability_freeze.py::test_the_barrel_reexport_edges_exist_at_head` |
| Every repo-root module and script is scanned | `tests/test_issue_149_legacy_reachability_freeze.py::test_every_repo_root_module_and_script_is_scanned` |
| **Mutation:** a hand-rolled httpx/urllib POST is reported | `tests/test_issue_149_legacy_reachability_freeze.py::test_a_hand_rolled_http_client_is_reported` |
...
| **Mutation:** a mention nested in a classified call is not swallowed | `tests/test_issue_149_legacy_reachability_freeze.py::test_a_watched_mention_nested_in_a_classified_call_is_not_swallowed` |
| **Precision:** the Component-endpoint marker is a path segment | `tests/test_issue_149_legacy_reachability_freeze.py::test_the_component_endpoint_marker_is_a_path_segment_not_a_substring` |
...
| **Mutation:** a simple assignment alias carries transitive identity | `tests/test_issue_149_legacy_reachability_freeze.py::test_a_simple_assignment_alias_carries_transitive_identity` |
| **Mutation:** re-pointing a composed HTTP target breaks the freeze | `tests/test_issue_149_legacy_reachability_freeze.py::test_repointing_a_composed_http_target_breaks_the_freeze` |
...
| The served ordering claim is scoped to what the scanner does | `tests/test_issue_149_legacy_reachability_freeze.py::test_the_scan_contract_scopes_its_ordering_claim_to_what_it_does` |
...
`python_source_count` moved 216 → 217: this slice added one scanned source, the gate-round
archiver under `scripts/`. That is a real addition to the scanned surface, not drift, and the
census gained no row for it — the archiver touches no legacy caller.

Fifty census rows and twenty-two ledger rows changed in exactly one field, `evidence_line`, all by
the same offset: documentation added inside `integration_builder.py` shifted the lines beneath it.
No row was added, removed or re-dispositioned, and no symbol, path or owning issue moved — verified
...
The slice's correction round moved lines again, and again only lines. Two served artifacts changed —
the capability catalog and the registered-description digest — because the monitoring tool gained an
action on its discovery surface and its docstring was completed from ten actions to all eighteen.
Thirty-eight §11 rows across §11.2, §11.3 and §11.4 then shifted their `evidence_line` by exactly
four, uniformly, which is the four lines that docstring expansion added to `server.py`. No row was
added, removed or re-dispositioned; no symbol, path, owning issue or disposition moved; and no
census leaf changed at all. Verified by diffing the regenerated tables against the committed ones
line by line rather than by trusting that the summary named only two artifacts, which it did.
...
source-addressed. Editing a source file moves them only if the served CONTENT changes, and a
docstring that is never served — measured: none of its phrases appear on any of the five served
surfaces — changes nothing a caller can read.

## 12.8 Issue #157 — governance completion: retirements and the served-flows baseline

Issue #157 (M12.19) closed the T9 remainder of the plan §4 triage on the typed authoring surface.
Two durable artifacts landed beside this inventory; both are DATA and both survive #160's deletion
of the legacy producers.

**Retirement records** — `tests/fixtures/governance/issue_157/retirements/` (index `INDEX.json`,
frozen by `tests/_issue_157_retirements.py --freeze` from a pristine `ba1be9f` extraction, replayed
against HEAD by `--check` until #160). Each record carries the pair of archetype payloads differing
only in the candidate field, the sha256 of every emitted component's XML for both, the plan verdict
digest with the spec echo stripped, the spec-echo paths that moved, and the non-vacuity control.

| Record | Legacy spelling | Decision | Typed-surface disposition |
| --- | --- | --- | --- |
| RET-157-01 | `jdbc_options` | RETIRE | refused by name |
| RET-157-02 | `fetch_size` | RETIRE | refused by name |
| RET-157-03 | `link_element` | RETIRE | refused by name |
| RET-157-04 | `sql_type` | RETIRE | refused by name |
| RET-157-05 | `direction` | RETAIN (moves the read-profile XML) | never on the typed surface |
| RET-157-06 | `cron` | RETIRE | refused by name; schedule activation is a future topology capability |
| RET-157-07 | `run_metadata` | RETIRE | refused by name; recorded runtime hint instead |
| RET-157-08 | query parameter `value_source=watermark` | RETIRE (additive spelling emitting nothing) | the typed watermark declaration's query-parameter references |
| RET-157-09 | `runtime_hints` | RETIRE | refused by name; recorded runtime hint instead |
| RET-157-10 | the legacy watermark object (`dpp_name`, `store_ref`, `persistence=dpp`) | SPLIT | the typed watermark declaration is the sole spelling; persistence stays the canonical persisted-property node |

The legacy archetype contracts keep accepting every spelling above until #160's cutover; the
refusal is on the typed per-root envelopes only (`GOVERNANCE_RETIRED_SPELLING`).

**Served-flows baseline** — `tests/fixtures/governance/issue_157/` (manifest `MANIFEST.json`,
frozen by `tests/_issue_157_flows_accounting.py --freeze`). Twenty-one cases over all seven legacy
`flows` producers, each case's complete ORDERED flows sequence and endpoint/provenance sequence
frozen with the immutable legacy input that produced it, 83 flow rows and 58 endpoint rows keyed
`(producer, case_id, key, kind)`. Nine rows — the DB-source and API-source rich `transform` rows —
are discharged by full normalized-payload equality against the canonical projection of a committed
canonical replay fixture (`input.canonical.json`, derived once by
`tests/_issue_157_canonical_fixtures.py` and never executed at replay); the remaining 132 rows are
`pending:#159`, pinned by count. #160 must list these fixtures on its deleted-name allowlist: they
carry the legacy producer names as frozen data.

## 12.9 Served-artifact movement (issue #184)

Issue #184 (M12.24) changed behaviour on the canonical authoring route, so the freeze moved for
recorded causes rather than by drift. `tests/_m12_12_legacy_inventory.py --write` was run on the
slice's tree, and the §11.2–§11.6 tables above were replaced with its `--emit-markdown` output in the
same change.

**Keyed diff against the previous baseline.** Rows are keyed by identity, not by list position: an
index-keyed diff reports every row after an insertion as changed, and read that way this
rebaseline looks like 2 275 changed leaves.

- **Census: two rows added, none removed, none otherwise changed.** Both are `legacy_transitive_call`
  edges in `src/boomi_mcp/categories/deployment/orchestration.py`: `_standalone_entry_refusal` calls
  `_root_entry`, and `orchestrate_deploy_action` calls `_standalone_entry_refusal`. The new refusal
  classifies the deploy target through the same entry authority the listener checks use, so it
  inherits their legacy-bearing edge. It implements #184 amendment 3 §8: a direct test run or
  schedule of a passthrough root that needs a caller is refused before any mutation.
- **Ledger: one row added, `LG-8e5effa6`** (the first edge), and **one row changed, `LG-ab50f45d`**,
  whose `sites` moved 5 → 6 for the second edge. Every other difference is an `evidence_line` moved
  by edits above it.
- **Eight served artifacts moved:** `SS-CAPABILITY-CATALOG:authoring_contract`; the
  `AuthoringRequestV1`, `AuthoringPlanResultV1`, `AuthoringCompileResultV1`, `IntegrationSpecV1` and
  `recipe_contributions` schema templates; the `database_to_api_sync` process template; and
  `SS-SCHEMA-TEMPLATES:walked_surface_digest`. What the slice changed on those surfaces:
  - the authoring contract's cache-emission, child-contract and prefix prose;
  - the ProcessIR schema's model descriptions;
  - the process template's `doccacheremove` mode, moved from supported to refused;
  - the recipe-layer modules `authoring/workflow.py` and `recipes/engine.py`, which now derive child
    entry contracts;
  - the capability revision that several of these artifacts re-serve.
- **Revisions, branch point `cbab28f` → the tree this rebaseline was taken from:**
...
**Stage-2 correction batch 3 (QA-184-s1-r3-01, CDX-184-r2-01..04, SELF-184-01) re-ran it.** Batch 2
changed validation behaviour, cache identity and forwarding obligations, without moving any revision.
Review round r2 measured that (CDX-184-r2-04): no behaviour oracle exercised an alias, an effect
declaration or a forwarding call. Batch 3 records each key's declared binding on its symbol, compares
components through that binding, gathers a cache's declarations across its references, marks every
cache a call may write, and derives inherited writers per property. It adds two behaviour oracle rows,
`component_identity` and `child_forwarding`, to the compiler revision. The same five served artifacts
moved as in batch 1, re-serving the new revisions. The census, the ledger rows, the contract snapshot
and `schema_revision` did not move. The §11 tables above were re-spliced from `--emit-markdown`, because
...
`INTEGRATION_COMPONENT_WRITE_CONFLICT`, so every other reference to a bound component carries the one
write's facts; a `reference_only` update is a write, as apply performs it. It compares every declared
effect, and derives a child's subprocess summary, with each cache key in its canonical spelling, and the
served `PROCESS_IR_SEMANTIC_PROFILE_MISMATCH` message now names an operation's profile reference, which
moves the authoring contract snapshot as well. The census, the ledger rows and `schema_revision` did not
move, and the §11 tables were re-spliced for moved `Baseline line` values. Revisions after batch 6:
`capability_revision` `sha256:d9e2a714…` and `compiler_revision` `sha256:af35ed93…`; `schema_revision` stays `sha256:c9583e6b…`.

**Stage-2 correction batch 7 (QA-184-s1-r7-01..02) re-ran it.** Batch 7 makes one canonical component
id the spelling every binding, write-conflict check and apply target uses (whitespace stripped, a GUID
lowercase), and computes every symbol fact of a reference apply does not write from the spec that writes
...
**Stage-2 correction batch 9 (CDX-184-r8-01..03, QA-184-s1-r9-01) re-ran it.** Batch 9 refuses a component the
request writes that more than one spec names, and no longer describes any spec by another's configuration.
The census, the ledger rows and `schema_revision` did not move, and the §11 tables were re-spliced for moved
`Baseline line` values. Revisions after batch 9: `capability_revision` `sha256:f8137554…` and `compiler_revision` `sha256:c946726d…`;
`schema_revision` stays `sha256:c9583e6b…`.

**Stage-2 correction batch 10 (QA-184-s1-r10-01) re-ran it.** Batch 10 hands the component bindings a route
resolves with its account to the write-conflict check and component identity, so a spec bound by name is
the component apply binds. The census, the ledger rows and `schema_revision` did not move, and the §11
tables were re-spliced for moved `Baseline line` values. Revisions after batch 10: `capability_revision`
`sha256:521730c3…` and `compiler_revision` `sha256:99a0c28d…`; `schema_revision` stays `sha256:c9583e6b…`.

**Stage-2 correction batch 11 (QA-184-s1-r11-01) re-ran it.** Batch 11 makes the typed plan block when an
account is in hand and the component plan could not be built, instead of judging component identity from
declared ids. The census, the ledger rows and `schema_revision` did not move, and the §11 tables were
re-spliced for moved `Baseline line` values. Revisions after batch 11: `capability_revision` `sha256:28936bcc…` and
`compiler_revision` `sha256:99a0c28d…`; `schema_revision` stays `sha256:c9583e6b…`.

**Stage-2 correction batch 12 (QA-184-s1-r12-01, CDX-184-r11-01) re-ran it.** Batch 12 serves an unbuildable
component plan as unjudged instead of blocking (the owner decision of 2026-09-14), and makes the compiler
revision witness the plan route's identity readings, derived from the route's imports. The census, the ledger rows
...
**Architect-review correction batch 18 (ARCH-184-r1-01 to -09) re-ran it.** Batch 18 corrects the nine findings of
the §6 architect review's first evaluation: one cache-content judgement for every typed consumer, the call prefix
carried on the path, forwarded unknown consumption, cached document properties and child guarantees across the call
boundary, the raw route's trusted context, the sink renderers' own wire refusal, and revision coverage derived from the
