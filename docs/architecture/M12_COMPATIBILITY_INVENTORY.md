...
| LG-f5622f58 | legacy_transitive_call | server.py | invoke_boomi_api | 1 | 3413 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-4ce172f4 | legacy_transitive_call | server.py | orchestrate_deploy | 3 | 4314 | #160 | follow the callee's row; this is an edge, not a site |
...
| LG-efa0b8f0 | http_client_call | server.py | web_callback | 1 | 5384 | #160 | WRT-external-transport-oauth-callback: unchanged — assert the target host/path is the configured token endpoint, never /Component. |
...
| LG-c3bf6d69 | raw_api_invoker | server.py | invoke_boomi_api | 1 | 3413 | #160 | WRT-raw-api-component: ONE canonical endpoint parser feeds classification, ID extraction AND transport; the reserved literal `bulk` is matched BEFORE the `<id>` arm and is never a componentId; every update-shaped call runs the two-sided process check. |
