...
def manage_component(
    ...
    config: Optional[str] = None,  # JSON string with action-specific fields
    config_yaml: Optional[str] = None,  # YAML for process creation (delegates to manage_process)
) -> dict:
    """Manage component lifecycle - all write operations.

    Actions:
    - create: Create component from XML template (config.xml) or delegate
              process creation via config_yaml to manage_process.
              Boomi has 25+ component types with unique XML schemas — use
              query_components get action on an existing component to obtain
              a valid XML template, modify it, then pass as config.xml.
    - update: Update component metadata (name, description, folder) or
              replace full XML via config.xml. Fetches current XML, patches
              fields, and PUTs back.
    - clone:  Clone existing component with a new name. Fetches source XML,
              strips identity attributes, sets new name from config.name,
              and POSTs as new component.
    - delete: Delete component via metadata API. Dependent components are
